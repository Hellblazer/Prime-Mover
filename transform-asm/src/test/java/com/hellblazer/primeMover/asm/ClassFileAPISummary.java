...
/**
 * Summary and practical examples of Java 24 ClassFile API (JEP 484) for the Prime Mover project context.
 *
 * Key learnings about the ClassFile API:
 *
 * 1. IMMUTABLE DESIGN: All elements are immutable, making transformations safe 2. LAMBDA-BASED: Uses functional
 * programming style vs ASM's visitor pattern 3. COMPOSABLE: Easy to chain and combine transformations 4. TYPE-SAFE:
 * Better compile-time safety with modern Java features 5. INTEGRATED: Part of the JDK, always up-to-date with class
 * file format
 *
 * Migration Path for Prime Mover: ===============================
 *
 * CURRENT ASM APPROACH: - EntityGeneratorOriginal uses ClassWriter, MethodVisitor - Complex visitor pattern with state
 * management - Manual label and stack management - Separate constant pool handling
 *
 * POTENTIAL CLASSFILE API APPROACH: - Use ClassFile.build() for generation - Lambda-based transformations - Automatic
 * label and stack management - Simplified constant handling
 *
 * BENEFITS FOR PRIME MOVER: - Reduced complexity in EntityGeneratorOriginal - Better maintainability and readability -
 * Automatic handling of complex bytecode details - Future-proof against class file format changes - No external ASM
 * dependency
 *
 * CONSIDERATIONS: - Requires Java 24+ (currently preview in 22/23) - Learning curve for team familiar with ASM - Need
 * to validate performance compared to ASM - Migration effort for existing code
 */
    ...
    public static void demonstrateBasicClassGeneration() {
        ...
        byte[] classBytes = cf.build(ClassDesc.of("com.example.SimpleClass"), classBuilder -> {
            classBuilder.withFlags(ClassFile.ACC_PUBLIC)
                        ...
                        .withMethodBody("<init>", MethodTypeDesc.of(ConstantDescs.CD_void), ClassFile.ACC_PUBLIC,
                                        codeBuilder -> {
                                            codeBuilder.aload(0)
                        ...
                        .withMethodBody("getValue", MethodTypeDesc.of(ConstantDescs.CD_int), ClassFile.ACC_PUBLIC,
                                        codeBuilder -> {
                                            codeBuilder.aload(0).getfield(ClassDesc.of("com.example.SimpleClass"),
                                                                          "value", ConstantDescs.CD_int).ireturn();
                                        });
        });
        ...
        try {
            ...
            byte[] transformedBytes = cf.transformClass(cf.parse(originalBytes),
                                                        ClassTransform.endHandler(classBuilder -> {
                                                            classBuilder.withField("generatedField",
