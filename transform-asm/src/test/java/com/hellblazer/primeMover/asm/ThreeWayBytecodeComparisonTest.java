...
/**
 * Test that compares bytecode generation between the original EntityGeneratorOriginal, EntityGeneratorRefactored, and
 * EntityGenerator to ensure they produce structurally equivalent results.
 */
public class ThreeWayBytecodeComparisonTest {
    ...
    public void testStructuralEquivalence() throws Exception {
        ...
        // Set up common data
        var transform = new SimulationTransformRefactored(
        ...
        EntityGeneratorRefactored refactoredGeneratorWithTimestamp = new EntityGeneratorRefactored(entity, events,
                                                                                                   timestamp);
    ...
    private void findFirstDifference(String comparison, byte[] bytes1, byte[] bytes2) {
        ...
        for (int i = 0; i < minLength; i++) {
            if (bytes1[i] != bytes2[i]) {
                System.out.printf("%s: First difference at index %d: 0x%02X vs 0x%02X%n", comparison, i,
                                  bytes1[i] & 0xFF, bytes2[i] & 0xFF);
