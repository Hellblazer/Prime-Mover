...
public class SimulationTransformTest {
    ...
    public void testGeneratorsStructuralEquivalence() throws Exception {
        ...
        originalGenerators.forEach((classInfo, originalGenerator) -> {
            ...
            var classFileGenerator = classFileGenerators.entrySet().stream()
                .filter(entry -> entry.getKey().getName().equals(classInfo.getName()))
                .map(entry -> entry.getValue())
                .findFirst()
                .orElse(null);
            ...
            try {
                ...
                assertStructuralEquivalence(originalBytecode, refactoredBytecode, classInfo.getName(), "Original vs Refactored");
                assertStructuralEquivalence(refactoredBytecode, classFileBytecode, classInfo.getName(), "Refactored vs ClassFile API");
                assertStructuralEquivalence(originalBytecode, classFileBytecode, classInfo.getName(), "Original vs ClassFile API");
            } catch (Exception e) {
    ...
    public void testTransformedClassesEquivalence() {
        ...
        originalTransformed.forEach((classInfo, originalBytecode) -> {
            ...
            var classFileBytecode = classFileTransformed.entrySet().stream()
                .filter(entry -> entry.getKey().getName().equals(classInfo.getName()))
                .map(entry -> entry.getValue())
                .findFirst()
                .orElse(null);
    ...
    public void testEntityInterfacesExtraction() {
        ...
        generators.keySet().forEach(classInfo -> {
            ...
            assertEquals(refactoredInterfaces.size(), classFileInterfaces.size(),
                "Refactored and ClassFile API should extract same number of entity interfaces for " + classInfo.getName());
            ...
            var refactoredNames = refactoredInterfaces.stream()
                .map(ci -> ci.getName())
                .collect(Collectors.toSet());
            var classFileNames = classFileInterfaces.stream()
                .map(ci -> ci.getName())
                .collect(Collectors.toSet());
            ...
            assertEquals(refactoredNames, classFileNames,
                "Refactored and ClassFile API should extract same entity interfaces for " + classInfo.getName());
        });
    }
    ...
    private void assertStructuralEquivalence(byte[] bytecode1, byte[] bytecode2, String className, String comparison) {
        ...
        assertEquals(class1.name, class2.name, 
            comparison + ": Class names should match for " + className);
        ...
        assertEquals(class1.interfaces, class2.interfaces, 
            comparison + ": Interfaces should match for " + className);
        ...
        if (className.contains("Template")) {
            ...
            for (int i = 0; i < class1.methods.size(); i++) {
                ...
                assertEquals(method1.desc, method2.desc, 
                    comparison + ": Method descriptor should match for method " + method1.name + " in " + className);
                assertEquals(method1.access, method2.access, 
                    comparison + ": Method access flags should match for method " + method1.name + " in " + className);
            }
        }
    }
    ...
    private boolean containsKronosReference(byte[] bytecode) {
        ...
        try {
            ...
            return bytecodeString.contains("Kronos") || 
                   bytecodeString.contains(com.hellblazer.primeMover.Kronos.class.getCanonicalName());
        } catch (Exception e) {
            return false;
        }
    }
