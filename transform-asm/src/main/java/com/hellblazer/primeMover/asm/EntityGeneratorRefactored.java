 ...
 *
 * This class generates the bytecode necessary to convert regular Java classes annotated with @Entity into
 * simulation-aware entities that can participate in the PrimeMover discrete event simulation framework.
 *
...
public class EntityGeneratorRefactored {
    ...
    private void boxPrimitive(GeneratorAdapter adapter, Type type) {
        switch (type.getSort()) {
            case Type.BYTE -> adapter.invokeStatic(Type.getType(Byte.class),
                                                   methodAccessor.valueOfMethods.get(Byte.class));
            case Type.CHAR -> adapter.invokeStatic(Type.getType(Character.class),
                                                   methodAccessor.valueOfMethods.get(Character.class));
            case Type.DOUBLE -> adapter.invokeStatic(Type.getType(Double.class),
                                                     methodAccessor.valueOfMethods.get(Double.class));
            case Type.FLOAT -> adapter.invokeStatic(Type.getType(Float.class),
                                                    methodAccessor.valueOfMethods.get(Float.class));
            case Type.INT -> adapter.invokeStatic(Type.getType(Integer.class),
                                                  methodAccessor.valueOfMethods.get(Integer.class));
            case Type.LONG -> adapter.invokeStatic(Type.getType(Long.class),
                                                   methodAccessor.valueOfMethods.get(Long.class));
            case Type.SHORT -> adapter.invokeStatic(Type.getType(Short.class),
                                                    methodAccessor.valueOfMethods.get(Short.class));
            case Type.BOOLEAN -> adapter.invokeStatic(Type.getType(Boolean.class),
                                                      methodAccessor.valueOfMethods.get(Boolean.class));
    ...
    private Map<String, String> buildMethodMappings() {
        ...
        for (var mi : remapped) {
            String key = METHOD_REMAP_KEY_TEMPLATE.formatted(clazz.getName().replace('.', '/'), mi.getName(),
            ...
            if (superClass != null && isSuperEntityMethod(superClass, mi)) {
                String superKey = METHOD_REMAP_KEY_TEMPLATE.formatted(superClass.getName().replace('.', '/'),
    ...
    private String buildMethodSignature(MethodInfo methodInfo) {
        ...
        signature.append('<').append(type.getClassName()).append(": ").append(
        methodInfo.getTypeDescriptor().getResultType()).append(" ").append(methodInfo.getName()).append('(');
     ...
     */
    private void generateEventMethod(Method method, int access, String name, String descriptor, String[] exceptions,
                                     ClassVisitor cv) {
        var exceptionTypes = exceptions == null ? null : Arrays.stream(exceptions).map(Type::getObjectType).toArray(
        Type[]::new);
        ...
        if (Type.VOID_TYPE.equals(method.getReturnType())) {
            var postMethod = blocking.contains(method) ? methodAccessor.postContinuingEventMethod
                                                       : methodAccessor.postEventMethod;
    ...
    private void generateInvokeCase(GeneratorAdapter adapter, int key) {
        ...
        if (remapped.contains(methodInfo)) {
            ...
            adapter.visitMethodInsn(Opcodes.INVOKESPECIAL, clazz.getSuperclass().getName().replace('.', '/'),
                                    remappedName, methodInfo.getTypeDescriptorStr(), methodInfo.isAbstract());
        }
    ...
    private int[] getEventKeys() {
        return mapped.entrySet().stream().filter(entry -> remapped.contains(entry.getValue())).mapToInt(
        Map.Entry::getKey).sorted().toArray();
    }
    ...
    private boolean isSuperEntityMethod(ClassInfo superClass, MethodInfo mi) {
        return superClass.getAnnotationInfo(Entity.class) != null && superClass.getMethodAndConstructorInfo()
                                                                               .stream()
                                                                               .anyMatch(e -> methodMatches(e, mi));
    }

    private boolean methodMatches(MethodInfo e, MethodInfo mi) {
        return mi.getName().equals(e.getName()) && mi.getTypeDescriptor().equals(e.getTypeDescriptor());
    }
    ...
    private void unboxBaseType(GeneratorAdapter adapter, char typeChar) {
        switch (typeChar) {
            ...
            case 'C' -> {
                ...
                adapter.invokeVirtual(Type.getType(Character.class),
                                      methodAccessor.valueAccessMethods.get(Character.class));
            }
            case 'D' -> {
            ...
            case 'I' -> {
                ...
                adapter.invokeVirtual(Type.getType(Integer.class),
                                      methodAccessor.valueAccessMethods.get(Integer.class));
            }
            case 'J' -> {
            ...
            case 'Z' -> {
                ...
                adapter.invokeVirtual(Type.getType(Boolean.class),
                                      methodAccessor.valueAccessMethods.get(Boolean.class));
            }
    ...
    private static class MethodAccessor {
        ...
        MethodAccessor() {
            ...
            this.postContinuingEventMethod = getMethod(Devi.class, POST_CONTINUING_EVENT, EntityReference.class,
    ...
    private class EventTransformVisitor extends ClassVisitor {
        ...
        public EventTransformVisitor(ClassWriter classWriter, Map<String, String> methodMappings,
                                     SimpleRemapper apiRemapper) {
        ...
        private MethodVisitor createMethodVisitor(int access, String name, String descriptor, String signature,
                                                  String[] exceptions) {
            ...
            super.visitMethod(access, remappedName, descriptor, signature, exceptions), name, descriptor);
        }
        ...
        private class MethodRemappingVisitor extends MethodVisitor {
            ...
            public void visitMethodInsn(int opcode, String owner, String name, String descriptor, boolean isInterface) {
                String newName = name;
                if (Opcodes.INVOKESPECIAL == opcode && name.equals(originalName) && descriptor.equals(
                originalDescriptor)) {
                    newName = methodMappings.getOrDefault(METHOD_REMAP_KEY_TEMPLATE.formatted(owner, name, descriptor),
                                                          name);
                }
