...
public class EntityGenerator {
    ...
    private final SootClass devi            = Scene.v()
                                                   .loadClass("com.hellblazer.primeMover.runtime.Devi",
                                                              SootClass.SIGNATURES);
    private final SootClass entity;
    private final SootClass entityReference = Scene.v()
                                                   .loadClass("com.hellblazer.primeMover.runtime.EntityReference",
                                                              SootClass.SIGNATURES);
    private final SootClass framework       = Scene.v()
                                                   .loadClass("com.hellblazer.primeMover.runtime.Framework",
                                                              SootClass.SIGNATURES);
    ...
    private final SootClass                noSuchMethodError = Scene.v()
                                                                    .loadClass(NoSuchMethodError.class.getCanonicalName(),
                                                                               SootClass.SIGNATURES);
    private final SootClass                object            = Scene.v()
                                                                    .loadClass(Object.class.getCanonicalName(),
                                                                               SootClass.SIGNATURES);
    private final SootClass                string            = Scene.v()
                                                                    .loadClass(String.class.getCanonicalName(),
                                                                               SootClass.SIGNATURES);
    ...
    public EntityGenerator(List<SootClass> generated, SootClass base, boolean validate) {
        this.base = base;
        this.validate = validate;
        entity = new SootClass(base.getJavaPackageName() + "." + base.getJavaStyleName() + GENERATED_ENTITY_SUFFIX,
                               Modifier.PUBLIC);
    ...
    protected void constructInvokeMap() {
        ...
        for (SootMethod method : inverseInvokeMap.values()) {
            if (method.getReturnType() != VoidType.v() && !isMarkedBlocking(method)) {
                markBlocking(method);
                log.info(String.format("Inferred blocking status of %1s.  Marking method as @Blocking", method));
            }
        }
    }

    /**
     * Generate the static class initialization method. This method initializes the
     * map of event method signatures to their ordinal number.
     * 
     * Method body:
     * 
    ...
    protected SootMethod generateClassInitMethod() {
        ...
        helper.assign(map, Jimple.v().newNewArrayExpr(string.getType(), IntConstant.v(inverseInvokeMap.size())));
        helper.assignStaticVariable(EVENT_MAP, map);
        for (Map.Entry<Integer, SootMethod> entry : inverseInvokeMap.entrySet()) {
            helper.assign(Jimple.v().newArrayRef(map, IntConstant.v(entry.getKey())),
    ...
    protected SootMethod generateConstructor(SootMethod constructor, SootMethod initializeMethod) {
        MethodHelper helper = new MethodHelper(entity, constructor.getName(), constructor.getParameterTypes(),
                                               VoidType.v(), constructor.getModifiers());
        helper.invoke(Jimple.v()
                            .newSpecialInvokeExpr(helper.thisLocal(), constructor.makeRef(), helper.getParameters()));
        helper.invoke(Jimple.v().newVirtualInvokeExpr(helper.thisLocal(), initializeMethod.makeRef()));
        helper.returnVoid();
        entity.addMethod(helper.getMethod());
        if (validate) {
            helper.validate();
    ...
    protected SootMethod generateEvent(SootMethod event, SootMethod initializeMethod) {
        MethodHelper helper = new MethodHelper(entity, event.getName(), event.getParameterTypes(),
                                               event.getReturnType(), event.getModifiers());

        Local initialized = helper.newLocal("initialized", BooleanType.v());
        helper.assignInstanceVariableTo(initialized, INITIALIZED_FIELD);
        UnitBox target = helper.generateIf(Jimple.v().newEqExpr(initialized, IntConstant.v(1)));
        helper.invoke(Jimple.v().newVirtualInvokeExpr(helper.thisLocal(), initializeMethod.makeRef()));
        ...
        Local arguments = helper.newLocal("arguments", ArrayType.v(object.getType(), 1));
        ...
        if (isMarkedBlocking(event)) {
            Local continuationReturnValue = helper.newLocal("continuationReturnValue", object.getType());
            helper.assign(continuationReturnValue,
                          Jimple.v()
                                .newVirtualInvokeExpr(controller,
                                                      devi.getMethod(FRAMEWORK_POST_CONTINUING_EVENT_SIGNATURE)
                                                          .makeRef(),
                                                      asList(helper.thisLocal(), IntConstant.v(invokeMap.get(event)),
                                                             arguments)));
    ...
    protected SootMethod generateInitializeMethod() {
        ...
        MethodHelper helper = new MethodHelper(entity, INITIALIZE_METHOD_NAME, Collections.EMPTY_LIST, VoidType.v(),
        ...
        UnitBox target = helper.generateIf(Jimple.v().newEqExpr(initializedLocal, IntConstant.v(0)));
        helper.returnVoid();

        /*
         * __controller = Framework.getController();
         */
        SootMethod getController = framework.getMethod("getController", Collections.emptyList(), devi.getType());
        Local controllerLocal = helper.newLocal("controller", devi.getType());
        helper.assign(controllerLocal, Jimple.v().newStaticInvokeExpr(getController.makeRef()));
        target.setUnit(helper.getLast());
    ...
    protected Unit generateInvoke(SootMethod event, Local arguments, MethodHelper helper, String tempPrefix) {
        ...
        for (int i = 0; i < event.getParameterCount(); i++) {
            Local rawArg = helper.newLocal(tempPrefix + "-raw" + i, object.getType());
            ...
            helper.assign(rawArg, Jimple.v().newArrayRef(arguments, IntConstant.v(i)));
            if (parameterType instanceof PrimType) {
                // unbox instructions
                helper.unbox((PrimType) parameterType, arg, rawArg);
            } else {
                // <Type> arg = (<Type>) rawArg;
                helper.assign(arg, Jimple.v().newCastExpr(rawArg, parameterType));
            }
        }

        if (event.getReturnType().equals(VoidType.v())) {
            helper.invoke(Jimple.v().newSpecialInvokeExpr(helper.thisLocal(), event.makeRef(), castArguments));
            helper.returnValue(NullConstant.v());
        } else {
            Local result = helper.newLocal("result", event.getReturnType());
            helper.assign(result, Jimple.v().newSpecialInvokeExpr(helper.thisLocal(), event.makeRef(), castArguments));
    ...
    protected SootMethod generateInvokeMethod() {
        MethodHelper helper = new MethodHelper(entity, INVOKE_METHOD_NAME, asList("eventOrdinal", "arguments"),
                                               asList(IntType.v(), ArrayType.v(object.getType(), 1)), object.getType(),
                                               Modifier.PUBLIC);
         ...
         * arguments[0], (AnotherArgType) arguments[1]...); break; } ... default: {
        ...
        for (Map.Entry<Integer, SootMethod> entry : inverseInvokeMap.entrySet()) {
            ...
            targets.add(generateInvoke(entry.getValue(), helper.getParameter(1), helper,
                                       tempPrefix + entry.getKey().intValue()));
        }

        Local nsmInstance = helper.newLocal("nsmInstance", noSuchMethodError.getType());

        // nsmInstance = new NoSuchMethodError();
        helper.assign(nsmInstance, Jimple.v().newNewExpr(noSuchMethodError.getType()));
        Unit defaultTarget = helper.getLast();
        helper.invoke(Jimple.v()
                            .newSpecialInvokeExpr(nsmInstance,
                                                  noSuchMethodError.getMethod(NO_ARG_CONSTRUCTOR_SIGNATURE).makeRef(),
                                                  Collections.emptyList()));
        // throw nsmInstance;
        helper.add(Jimple.v().newThrowStmt(nsmInstance));

        helper.add(Jimple.v().newLookupSwitchStmt(helper.getParameter(0), lookupValues, targets, defaultTarget));
        target.setUnit(helper.getLast());
        entity.addMethod(helper.getMethod());
        if (validate) {
            helper.validate();
    ...
    protected SootMethod generateSignatureForMethod() {
        MethodHelper helper = new MethodHelper(entity, SIGNATURE_FOR_METHOD_NAME, asList(IntType.v()), string.getType(),
                                               Modifier.PUBLIC);
        ...
        helper.assign(signature, Jimple.v().newArrayRef(eventMap, helper.getParameter(0)));
    ...
    protected void populateFields() {
        ...
        entity.addField(new SootField(EVENT_MAP, stringArrayType, Modifier.STATIC | Modifier.PRIVATE));

        // private Kalachakra __controller;
        entity.addField(new SootField(CONTROLLER_FIELD, devi.getType(), Modifier.PRIVATE));

        // private boolean __initialized;
        entity.addField(new SootField(INITIALIZED_FIELD, BooleanType.v(), Modifier.PRIVATE));
    ...
    private int gatherAllMethods(Set<String> mapped, int index) {
        ...
        while (current != null && current != object) {
            for (SootMethod method : current.getMethods()) {
                if (!mapped.contains(method.getSubSignature()) && isEvent(method)) {
    ...
    private int gatherMethodsForInterface(SootClass iFace, Set<String> mapped, int index) {
    ...
    private boolean isEvent(SootMethod method) {
        return method.isPublic() && method.isConcrete() && !method.isStatic() && !isInit(method) &&
               !isMarkedNonEvent(method);
    }
