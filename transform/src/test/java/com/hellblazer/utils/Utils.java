...
public class Utils {
    ...
    /**
     * Copy the contents of the source file into the destination file using the supplied buffer
     *
     * @param sourceFile
     * @param destFile
     * @param buffer
    ...
    /**
     * Copy the contents of the source file into the destination file using a buffer of the supplied size
     *
     * @param sourceFile
     * @param destFile
     * @param bufferSize
    ...
    /**
     * Copy and transform the zip entry to the destination. If the transformation extensions contains the entry's
     * extension, then ${xxx} style parameters are replace with the supplied properties or System.getProperties()
     *
     * @param dest
     * @param zf
     * @param ze
     ...
     * @param targetLocation - if exists, must be a directory. Will be created with full paths if does not exist
     * @throws IOException
     */
    ...
    /**
     * Expand the zip resource into the destination, replacing any ${propName} style properties with the corresponding
     * values in the substitutions map
     *
     * @param zip           - the zip file to expand
     * @param extensions    - the list of file extensions targeted for property substitution
    ...
    /**
     * Find the substitution value for the key in the properties. If the supplied properties are null, use the system
     * properties.
     *
     * @param key
     * @param props
    ...
    /**
     * Remove and reinitialze the directory. The directory and full paths will be created if it does not exist
     *
     * @param directory
     */
    ...
    /**
     * Go through the input stream and replace any occurance of ${p} with the props.get(p) value. If there is no such
    ...
    /**
     * Try to resolve a "key" from the provided properties by checking if it is actually a "key1,key2", in which case
    ...
    /**
     * Resolve a resource. First see if the supplied resource is an URL. If so, open it and return the stream. If not,
     * try for a file. If that exists and is not a directory, then return that stream. Finally, look for a classpath
     * resource, relative to the supplied base class. If that exists, open the stream and return that. Otherwise, barf
     *
     * @param base     - the base class for resolving classpath resources - may be null
     * @param resource - the resource to resolve
     * @return the InputStream of the resolved resource
     * @throws IOException - if something gnarly happens, or we can't find your resource
     */
    ...
    /**
     * Resolve a resource. Replace any ${foo} style properties in the resource with the supplied properties map. First
     * see if the supplied resource is an URL. If so, open it and return the stream. If not, try for a file. If that
     * exists and is not a directory, then return that stream. Finally, look for a classpath resource, relative to the
     * supplied base class. If that exists, open the stream and return that. Otherwise, barf
     *
     * @param base       - the base class for resolving classpath resources - may be null
     * @param resource   - the resource to resolve
    ...
    /**
     * Transform the contents of the input stream, replacing any ${p} values in the stream with the value in the
     * supplied properties. The transformed contents are placed in the supplied output file.
     *
     * @param properties
     * @param extensions
     * @param is
