...
public class ClassFileAPIPatterns {
    ...
    public static void testBasicTransformation() {
        ...
        try {
            ...
            byte[] transformed = cf.transformClass(classModel, ClassTransform.endHandler(classBuilder -> {
    ...
    public static void testMethodGeneration() {
        ...
        try {
            byte[] classBytes = cf.build(ClassDesc.of("TestMethodGen"), classBuilder -> {
                classBuilder.withFlags(ClassFile.ACC_PUBLIC).withMethodBody("testMethod",
                                                                            MethodTypeDesc.of(ConstantDescs.CD_String,
                                                                                              ConstantDescs.CD_int),
                                                                            ClassFile.ACC_PUBLIC, codeBuilder -> {
                    codeBuilder.ldc("Method called with: ").areturn();
                });
            });
    ...
    public static void testTableSwitch() {
        ...
        try {
            byte[] classBytes = cf.build(ClassDesc.of("TestSwitch"), classBuilder -> {
                classBuilder.withFlags(ClassFile.ACC_PUBLIC).withMethodBody("switchMethod",
                                                                            MethodTypeDesc.of(ConstantDescs.CD_String,
                                                                                              ConstantDescs.CD_int),
                                                                            ClassFile.ACC_PUBLIC, codeBuilder -> {
                    ...
                    codeBuilder.tableswitch(0, 1, defaultLabel,
                                            java.util.List.of(SwitchCase.of(0, case0), SwitchCase.of(1, case1)));
