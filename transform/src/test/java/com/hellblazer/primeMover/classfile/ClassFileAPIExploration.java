 ...
 *
 * This class demonstrates the key features and capabilities of the new ClassFile API introduced in Java 24.
 */
public class ClassFileAPIExploration {
    ...
    public static void generateSimpleClass() throws Exception {
        ...
        byte[] classBytes = cf.build(generatedClassDesc, classBuilder -> {
            ...
            // Add constructor
            .withMethodBody("<init>", MethodTypeDesc.of(ConstantDescs.CD_void, stringDesc), ClassFile.ACC_PUBLIC,
                            codeBuilder -> {
                                codeBuilder.aload(0) // load 'this'
                                           .invokespecial(objectDesc, "<init>",
                                                          MethodTypeDesc.of(ConstantDescs.CD_void)).aload(
                                           0) // load 'this'
                                           .aload(1) // load parameter 'name'
                                           .putfield(generatedClassDesc, "name", stringDesc).return_();
                            })

            // Add getName() method
            .withMethodBody("getName", MethodTypeDesc.of(stringDesc), ClassFile.ACC_PUBLIC, codeBuilder -> {
                codeBuilder.aload(0) // load 'this'
                           .getfield(generatedClassDesc, "name", stringDesc).areturn();
            });
        });
    ...
    public static void parseClassFile() throws Exception {
        ...
        for (FieldModel field : classModel.fields()) {
            System.out.println("  " + field.fieldName().stringValue() + " : " + field.fieldType().stringValue());
        }
    ...
    public static void transformExistingClass() throws Exception {
        ...
        byte[] transformedBytes = cf.transformClass(cf.parse(originalBytes), ClassTransform.endHandler(classBuilder -> {
