...
public class ClassFileAPIComparison {
    ...
    public static byte[] addTransformedAnnotation(byte[] originalBytes, String timestamp) {
        ...
        return cf.transformClass(cf.parse(originalBytes), ClassTransform.endHandler(classBuilder -> {
    ...
    public static byte[] createClassWithClassFileAPI() {
        ...
        return cf.build(ClassDesc.of("com.example.Generated"), classBuilder -> {
            classBuilder.withFlags(ClassFile.ACC_PUBLIC)
                        ...
                        .withMethodBody("<init>", MethodTypeDesc.of(ConstantDescs.CD_void, ConstantDescs.CD_int),
                                        ClassFile.ACC_PUBLIC, codeBuilder -> {
                            codeBuilder.aload(0).invokespecial(ConstantDescs.CD_Object, "<init>",
                                                               MethodTypeDesc.of(ConstantDescs.CD_void)).aload(0).iload(
                            1).putfield(ClassDesc.of("com.example.Generated"), "value", ConstantDescs.CD_int).return_();
                        })
                        .withMethodBody("getValue", MethodTypeDesc.of(ConstantDescs.CD_int), ClassFile.ACC_PUBLIC,
                                        codeBuilder -> {
                                            codeBuilder.aload(0).getfield(ClassDesc.of("com.example.Generated"),
    ...
    public static byte[] transformMethodCalls(byte[] originalBytes, String fromClass, String toClass) {
        ...
        return cf.transformClass(cf.parse(originalBytes), ClassTransform.transformingMethods(
        MethodTransform.transformingCode((codeBuilder, element) -> {
            switch (element) {
                case InvokeInstruction invoke when invoke.owner().asInternalName().equals(fromClass) -> {
                    // Transform the method call to use the new class
                    codeBuilder.invoke(invoke.opcode(), ClassDesc.of(toClass.replace('/', '.')),
