...
public class SimpleClassFileTest {
    ...
    public static void exploreClassFileAPI() {
        ...
        for (var method : classFileClass.getMethods()) {
            if (method.getDeclaringClass() == classFileClass || method.getDeclaringClass().getName().startsWith(
            "java.lang.classfile")) {
                System.out.println("  " + method.getName() + "(" + java.util.Arrays.stream(method.getParameterTypes())
