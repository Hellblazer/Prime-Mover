...
public class TestContinuationAnalysis {
    @Test
    public void testAnalysis() throws Exception {
        ...
        String[] argv = { "-w", "-process-dir", PROCESSED_DIR.getAbsolutePath(), "-v" };
        argv = new String[] { "-w" };
    ...
    private HashMap<String, byte[]> getOutputBits() throws IOException {
        ...
        classBits.put(className, getBits(new File(OUTPUT_DIR, className.replace('.', '/') + ".class")));
        return classBits;
    }
    ...
    private void populateProcessedDir() throws IOException {
        ...
        for (String clazz : new String[] { ContinuationPrototype.class.getSimpleName(),
                                           InferredContinuation.class.getSimpleName() }) {
            String classFileName = clazz + ".class";
            copy(new File(sourceDir, classFileName), new File(targetDir, classFileName));
        }
    }
}
