 ...
 * <br There are two possible ways to have other objects register changes of
 * another object's status: <ol <liCall the <code"update(String
 * updateStr)"</codemethod of an observer to pass the String to be counted
 * directly to one other object interested. Using the direct call allows easy
 * passing of a String to a single observer. No instance of this
 * <codeTextSupplier</codeclass is needed using direct passing of Strings.</li
 * <liUsing the <codeTextSupplier</codeto publish the information to interested
 * observers requires you to derive a class and implement the call of the
 * observer's <codeupdate(Observable, Object)</codemethod with the required
 * texts. This way of passing Strings involves more code to be written, but
 * offer the potential to have multiple observers and change the number of
 * observers during runtime. These options are not available when using the
 * simpler version above.</li </ol
 *
...
public abstract class TextSupplier extends java.util.Observable {
    ...
    /**
     * Indicates that this TextSupplier (as an Observable) has changed and notifies
     * all its observers of this change providing the modified text as parameter.
     * This is the "Push-model", thus pushing the String (in the parameter
     * <codearg</code) to the Observer (thus the <codeStatisticObject</code).
     *
     * @param arg Object : The Object wrapping the text with which the observers
     *            will be updated.
     */
     ...
     * be overridden by the user in a way that it provides the text (as a
