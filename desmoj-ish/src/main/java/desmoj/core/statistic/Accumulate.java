...
public class Accumulate extends desmoj.core.statistic.ValueStatistics {
    ...
    /**
     * Constructor for a Accumulate object that will not be connected to a
     * <code>ValueSupplier</code> automatically. An initial value stated explicitly
     * will be assumed after creation/reset.
     *
     * @param ownerModel   Model : The model this Accumulate is associated to
     * @param name         java.lang.String : The name of this Accumulate object.
    ...
    public double getMean() {
        ...
        if (this._pausedSince == null) {
            long periodCurrentValue = TimeOperations.diff(this.presentTime(), _lastUpdate).getTimeInEpsilon();
    ...
    public double getStdDev() {
        ...
        // update mean and sum of squares of... to reflect the period since the last
        // update
        // (only necessary if not paused at the moment)
        if (this._pausedSince == null) {
            long periodCurrentValue = TimeOperations.diff(this.presentTime(), _lastUpdate).getTimeInEpsilon();
            ...
            currentSumOfSquaredDevsFromMean += (getLastValue() - old_mean) * (getLastValue() - current_mean)
            * periodCurrentValue;
        }
    ...
    public void pause() {
        ...
        if (this._pausedSince != null) {
            sendWarning("Attempt to pause an Accumulate which is already paused." + " Method call will be ignored.",
                        "Accumulate: " + this.getName() + " Method: pause()",
