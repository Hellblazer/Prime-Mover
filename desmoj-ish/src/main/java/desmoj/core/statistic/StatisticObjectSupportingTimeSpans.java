...
/**
 * <code>StatisticObjectSupportingTimeSpans</code> class is the super class of
 * the specific data collector classes able to handle <code>TimeSpan</code>
 * objects. If the data collector should be used to measure the lengths of
 * <code>TimeSpan</code>s, the method
 * <code>setShowTimeSpansInReport(true)</code> should be called to appropriately
 * assign the (reference) unit.
 *
...
public abstract class StatisticObjectSupportingTimeSpans extends StatisticObject implements java.util.Observer {
    ...
    /**
     * Constructor for a StatisticObjectSupportingTimeSpans, preliminarily without a
     * unit assigned
     *
    ...
    /**
     * Sets if values should be interpreted and printed as TimeSpans (subject to the
     * experiment's reference unit) in the Report. Unless a unit is explicitly
     * otherwise using <code>setUnit(...)</code>, this class assumes updates refer
     * to the reference time unit.
     *
     * @param value boolean : true, if values should be printed as TimeSpans, false
     *              if not.
     */
