...
public class DiscreteDistPoisson extends DiscreteDist<Long> {
     ...
     * Giving zero or negative values for the mean value will result in zero being returned by the <code>sample()</code>
     * method.
     *
    ...
    public DiscreteDistPoisson(Model owner, String name, double meanValue, boolean showInReport, boolean showInTrace) {
        ...
        if (mean > 500) {
            ...
            "DiscreteDistPoisson : " + getName() + " Constructor: DiscreteDistPoisson(Model owner, String name,"
            ...
            + " with a mean 500 or less only.", "Use this class for poisson distributions with a smaller mean only.");
        }
    }
