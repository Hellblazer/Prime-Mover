...
public abstract class NumericalDist<N extends Number> extends Distribution {
    ...
    public NumericalDist(Model owner, String name, boolean showInReport, boolean showInTrace) {
    ...
    /**
     * Method to return the specific sample as a value of type N. Default
     * implementation calls <code>getInverseCumulativeProbability()</code>, to which
     * a random number from <code>randomGenerator</code> is passed. Subclasses may
     * need to refine this method if <code>getInverseCumulativeProbability()</code>
     * is not implemented as the distribution does not support inverse
     * transformation.
     *
    ...
    public N sample() {
        ...
        do {
            ...
            sample = this.antithetic ? getInverseOfCumulativeProbabilityFunction(1 - randomGenerator.nextDouble())
        ...
        if (nonNegative && sample.doubleValue() < 0) {
            sendWarning("NumericalDist that was set non-negative faild to sample a non-negative value."
            + "Returning a negative result.", "NumericalDist: " + this.getName() + " Method: public void sample()",
    ...
    public TimeSpan sampleTimeSpan() {
        ...
        if (!this.getModel().isConnected()) {
            sendWarning("Failed to sample a TimeSpan. Returning TimeSpan(0) instead.",
