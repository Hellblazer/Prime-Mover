...
public class EntityDistEmpirical<E extends Entity> extends EntityDist<E> {
     ...
     */
    public EntityDistEmpirical(Model owner, String name, boolean showInReport, boolean showInTrace) {
    ...
    public boolean add(E e, double probability) {
        ...
        if (probability < 0) {
            sendWarning("Can't add empirical entry! Command ignored.",
                        "EntityDistEmpirical " + getName() + " Method: void add (E e, double probability)",
                        "The probability parameter given is invalid: " + probability,
        ...
        if (this.contains(e)) {
            sendWarning("Can't add empirical entry! Command ignored.",
                        "EntityDistEmpirical " + getName() + " Method: void add (E e, double probability)",
                        "The entity " + e + " is already included in the distribution.",
                        "Be sure to only add entries that are not yet present. "
                        + "Method contains(e) can be used to verify entity e " + "is not yet included.");
            return false;
        }
    ...
    public void changeProbability(E e, double newProbability) {
        ...
        // probability must be positive
        if (newProbability < 0) {
            sendWarning("Can't change probability of entity " + e + "! Command ignored.",
                        "EntityDistEmpirical " + getName()
                        + " Method: void changeProbability (E e, double newProbability)",
                        "The probability parameter given is invalid: " + newProbability,
        ...
        if (!this.contains(e)) {
            sendWarning("Can't change probability of entity " + e + "! Command ignored.",
                        ...
                        + "Method contains(e) can be used to verify entity e " + "is included.");
            return;
        }
    ...
    public E sample() {
        ...
        if (!_isInitialized) {
            sendWarning("Invalid sample returned!", "EntityDistEmpirical : " + getName() + " Method: E sample()",
                        "The distribution has not been initialized properly yet: " + "No entity provided yet!",
                        "Be sure to pass at least one entity to be returned " + "to an entityDistUniform distribution."
                        + "Note that calling method isInitialized() " + "which returns a boolean telling "
