...
public class DiscreteDistEmpirical<N extends Number> extends DiscreteDist<N> {
     ...
     */
    public DiscreteDistEmpirical(Model owner, String name, boolean showInReport, boolean showInTrace) {

        super(owner, name, showInReport, showInTrace);
        _values = new java.util.ArrayList<>(); // Initialize Vector for
        // values
    ...
    public void addEntry(N value, double frequency) {
        ...
        if (frequency < 0) {
            sendWarning("Can't add empirical entry! Command ignored.",
                        "DiscreteDistEmpirical " + getName() + " Method: void addEntry (double value, double "
                        + "frequency)", "The frequency parameter given is invalid because it is negative: " + frequency,
    ...
    public N getInverseOfCumulativeProbabilityFunction(double p) {
        ...
        if (!_isInitialized) {
            sendWarning("Invalid sample returned!", "DiscreteDistEmpirical: " + getName() + " Method: double sample()",
