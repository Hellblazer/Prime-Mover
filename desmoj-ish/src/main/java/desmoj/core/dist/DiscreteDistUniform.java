...
public class DiscreteDistUniform extends DiscreteDist<Long> {
     ...
     * The specific lower and upper borders of the range of this distribution have
     * to be given here at creation time. Note that the lower border in fact has to
     * be lower than the upper border. If not, the parameters will be swapped to
     * assign the higher value to the upper border of the distribution. Two equal
     * values for upper and lower will be accepted, but result in a constant
     * distribution that will invariabbly return that given value. In both cases a
     * warning mesage will be produced.<br/>
     ...
     */
    public DiscreteDistUniform(Model owner, String name, long minValue, long maxValue, boolean showInReport,
                               boolean showInTrace) {
    ...
    /**
     * Returns the next uniformly distributed integer sample. The value returned is
     * basically the uniformly distributed pseudo random number produced by the
     * underlying random generator stretched to match the range specified by the
     ...
     * no sense to set a <code>UniformDiscreteDist</code> object with a negative
     * lower border to nonNegative. In this case a warning will be sent. <br>
    ...
    public void setNonNegative(boolean newValue) {
        if (newValue && _min < 0) {
            sendWarning("Attempt to set a UniformDiscreteDist distribution with a negative "
            + "lowerBorder to nonNegative. This will be done, but doesn't make sense!",
                        "DiscreteDistUniform: " + this.getName() + " Method: public void "
