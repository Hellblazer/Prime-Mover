...
public class DataListHistogramReporter extends Reporter {
    ...
    /**
     * Defines the maximal amount of classes found by the algorithm to calculate the
     ...
     * greatest value, the interval-size and the count of elements in the histogram.
     *
    ...
    /**
     * The histogram's dimensions, containing the lowest value, the greatest value,
     * the interval-size and the count of elements in the histogram.
     */
    private double[] _dimensions;

    /**
     * Constructor for a new DataListReporter. Note that although any Reportable is
     * accepted you should make sure that only subtypes of DataListTally.DataList
     * are passed to this constructor. Otherwise an incorrect output will be
     * produced.
     *
     ...
     */
    public DataListHistogramReporter(Reportable informationSource, double min, double max) {
        ...
        if (source instanceof DataListTally.DataList) {
            groupHeading = "Histogram for " + ((DataListTally.DataList) informationSource).getTallyName();
            numColumns = ((int) _dimensions[3] <= 1) ? 1 : (int) _dimensions[3] - 1;
            columns = new String[numColumns];
            entries = new String[numColumns];

            for (int i = 0; i < numColumns; i++) {
                columns[i] = "[" + (_dimensions[0] + (i * _dimensions[2])) + " - "
                + (_dimensions[0] + ((i + 1) * _dimensions[2])) + (i == numColumns - 1 ? "]" : ")");
            }
        }
    ...
    public String[] getEntries() {
        ...
        if (source instanceof DataListTally.DataList) {
            DataListTally.DataList.Element current = ((DataListTally.DataList) source).getFirst();
            int count = 0;
            int index = 0;
            ...
            while (current != null) {
                ...
                if (value < _dimensions[0] + ((index + 1) * _dimensions[2]) || index == (numColumns - 1)) {
                    count++;
