...
public class DebugFileOut extends TableOutput implements MessageReceiver {
    ...
    /**
     * Creates a DebugOut to print DebugNotes into a HTML page. By opening the file,
     * the necessary HTML tags to define a webpage are already inserted into the
     * file. The parameter given should reflect the experiment that produces this
     * file.
     *
    ...
    /**
     * Closes this DebugOut. Writes the final necessary HTML-tags to close a table
    ...
    /**
     * Opens a new file with the given filename for writing DebugNotes into a HTML
     * table. The output path is set to the working directory. If no filename is
     * given, the default filename "DESMOJ_debugfile.html" is used.
     *
     ...
     * "DESMOJ_debugfile.html" is used. If no pathname is given the current working
    ...
    /**
     * Receives a debugnote and writes its contents formatted to a HTML table into a
     * file in the user's default directory. Note that although any type of message
     * may be given to this method, only debugnotes will be processed. If other
    ...
    public void receive(Message m) {
        ...
        // check parameters
        // again nulls
        if ((m == null) || !(m instanceof DebugNote))
