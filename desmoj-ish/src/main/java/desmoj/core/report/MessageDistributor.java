 ...
 * MessageDistributor. When registering, the <code>MessageReceiver</code> has to
 * pass a type to identify which type of messages they want to have forwarded.
 * Note that multiple <code>MessageReceiver</code> s can be registered to get
 * the same type of messages as well as a <code>MessageReceiver</code> can be
 * registered with different types of messages, if it is capable of handling
 * such. This enables a modeller i.e. to get the error messages displayed on
 * screen additional to the file being stored on harddisk by default. This is
 * also handy if a simulation should be run as an Applet thus having no or
 * restricted disk access and using multiple scrollable windows instead.
 *
...
public class MessageDistributor implements MessageReceiver {
    ...
    /**
     * The inner class messagelink keeps track of the types of messages and their
     * related messagereceiver objects. Designed as an inner class to
    ...
    private static class MLink {
        ...
        /**
         * The Vector filled with messagereceivers registered to receive messages of the
         * attached type.
         */
        Vector<MessageReceiver> clients;

        /**
         * Flag to state whether the current type of message is being distributed or
         * currently not distributed. Can be set via <code>SwitchOn()</code> or
         * <code>SwitchOff()</code> methods.
         */
        boolean isOn;
        ...
        /**
         * Counts the number of future messages to be skipped. This is necessary to
         * blend out any model activities that are related to internal operations and
         * thus would confuse the modeller.
         */
        int skipCount;

        /**
         * Constructs a link with the given parameters. This is just a convenient
         * shorthand for setting up the parameters each at a time.
         */
    ...
    public MessageDistributor() {
        ...
        try {
            ...
            throw (new desmoj.core.exception.DESMOJException(new ErrorMessage(null,
     ...
     * messagereceiver.
     *
     * @param out MessageReceiver : The messagereceiver to be removed from all
     *            messages' lists of receivers
     */
    public void deRegister(MessageReceiver out) {

        // check parameter
        // invalid param, so just return

        if ((out == null) || (_head == null))
    ...
    /**
     * De-registers a messagereceiver object to stop receiving messages of the given
     * type. The given messagereceiver object is taken from the list of
     * messagereceiver receiving messages of the passed messagetype. If invalid
     * parameters are given (i.e. <code>null</code> references) this method simply
     * returns
     *
     * @param out         MessageReceiver : The messagereceiver to be de-registered
     * @param messageType java.lang.Class : The type of messages the messagereceiver
     ...
     *
     * @param out       MessageReceiver : The messagereceiver to be de-registered
     * @param className String : The type of messages the messagereceiver should be
     *                  deregistered from
     */
    ...
    /**
     * Checks if the current messagetype is switched on to be distributed. If not,
     * no messages of the given type are distributed or the given type of message is
     * not registered here.
     *
     * @return boolean : Is <code>true</code> if the type of message is distributed
     *         <code>false</code> if not or messagetype is not registered here
     */
     ...
     *
     * @return boolean : Is <code>true</code> if the messagetype is registered,
    ...
    /**
     * Receives a reporter and forwards it to all messagereceiver objects registered
     * with the type of reporter sent.
     *
     * @param r Reporter : The reporter to be forwarded to all registered
     *          messagereceivers.
     */
    @Override
    ...
    /**
     * Receives a message and forwards it to all messagereceiver objects registered
     * with the type of message sent. Messages are sent, if the type of message is
     * switched on and if the skipCounter is zero, thus not skipping any messages of
     * that type.
     *
     * @param m Message : The message to be forwarded to all registered
     *          MessageReceivers
     */
    @Override
     ...
     *
     * @param out       MessageReceiver : The messagereceiver to be registered
     * @param className java.lang.String : The name of the type of messages the
    ...
    public void register(MessageReceiver out, String className) {
        ...
        // check parameters
        // invalid param
        if ((out == null) || (className == null) || (className.length() == 0))
            return; // invalid param
    ...
    /**
     * Skips the transmission of the next tracenote or increases the skipCounter by
     * one. This is necessary to blend out any activities managed by the framework
     * that would otherwise confuse the modeller.
     */
     ...
     * skipCount by the given number. This is necessary to blend out any activities
     * managed by the framework that would otherwise confuse the modeller.
     *
    ...
    public void skip(Class<?> messageType, int skipNumber) {
        ...
        if ((skipNumber < 1) || (messageType == null))
            return;
    ...
    /**
     * Returns the messagelink for the given class or <code>null</code> if the class
