...
public class TraceNote extends Message {
     ...
     * <code>null</code> references are given, they are displayed as "----" in the
     * trace output.
     *
     ...
     */
    public TraceNote(Model origin, String message, TimeInstant time, Entity entityInvolved,
        ...
        if (entitiesInvolved == null || entitiesInvolved.isEmpty())
            ...
            if (entitiesInvolved.size() > 1) {
                _who += entitiesInvolved.size() > 2 ? ", " + entitiesInvolved.get(1).getQuotedName()
                                                    : " and " + entitiesInvolved.get(1).getQuotedName();
