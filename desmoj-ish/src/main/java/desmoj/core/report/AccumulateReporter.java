...
public class AccumulateReporter extends desmoj.core.report.Reporter {
    ...
    /**
     * Constructor for a new AccumulateReporter. Note that although any Reportable
     * is accepted you should make sure that only subtypes of Accumulate are passed
     ...
     * reporter.
     *
     * @param informationSource desmoj.core.simulator.Reportable : The Accumulate to
     *                          report about.
     */
    ...
    public String[] getEntries() {
        if (source instanceof desmoj.core.statistic.Accumulate) {
            ...
            if (TimeInstant.isEqual(acc.presentTime(), acc.resetAt()) || acc.getObservations() == 0) // OR no
                                                                                                     // observations are
            // made
            {
            ...
            // has no time passed since the last reset?
            if (TimeInstant.isEqual(acc.presentTime(), acc.resetAt()) || acc.getObservations() < 2) // OR not enough
            // observations
            // are made
            {
