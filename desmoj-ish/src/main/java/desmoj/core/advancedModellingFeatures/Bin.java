 ...
 * arbitrary choice, as the <ode>Bin</code> combines queueing and scheduling
 ...
 * determined by the user. <code>Bin</code> is derived from
...
public class Bin extends desmoj.core.simulator.QueueBased {
     ...
     * which are enqueued before that entity in the queue. Is <code>false</code> per
     * default.
     */
    ...
    /**
     * Weighted sum of available products in the Bin over the time (must be divided
    ...
    /**
     * Constructor for a <code>Bin</code> with a number of initial units of a
     * product in it. Waiting consumer processes are sorted (and thus activated)
     * according to their queueing priorities (highest first) and (if equal) by a
     ...
     * limit of the underlying queue can be chosen, too.
     *
     ...
     *                     capacity.
     * @param initialUnits long : The units of a product the Bin starts with. Must
     *                     be positive.
     ...
     */
    public Bin(desmoj.core.simulator.Model owner, String name, int sortOrder, int qCapacity, long initialUnits,
        ...
        if (initialUnits < 0) // there can't be less than nothing
        {
            ...
            + " units. Initial number of units set to zero!",
                        "Bin: " + getName() + " Constructor: Bin (desmoj.Model owner, "
                        + "String name, long initialUnits, boolean showInReport, " + "boolean showInTrace)",
                        "A negative number of units does not make sense here.",
                        "Make sure to initialize a Bin always with a positive number of " + "initialUnits.");
        ...
        if (qCapacity < 0) {
            ...
            + "A queue with unlimited capacity will be created instead.",
                        "Bin : " + getName() + " Constructor: Bin (desmoj.Model owner, String name, "
     ...
     * Constructor for a <code>Bin</code> with a number of initial units of a
     * product in it. Waiting consumer processes are sorted (and thus activated)
     * according to their queueing priorities (highest first) and (if equal) by
     * FIFO. The capacity limit of the underlying queue is unlimited.
     *
     * @param owner        Model : The model this Bin is associated to.
     * @param name         java.lang.String : The Bin's name
     * @param initialUnits long : The units of a product the Bin starts with. Must
     ...
     * @param showInTrace  boolean : Flag for trace to produce trace messages.
     */
    public Bin(desmoj.core.simulator.Model owner, String name, long initialUnits, boolean showInReport,
               boolean showInTrace) {
        super(owner, name, showInReport, showInTrace); // construct QueueBased
        reset();

        _queue = new QueueListFifo<>(); // make an actual queue and give it a
    ...
    /**
     * Returning the average number of products available in the bin over the time
     * since the last reset of the bin.
     *
     * @return double : The average number of products available in the bin over the
    ...
    public double avgAvail() {
        ...
        if (diff.isZero()) // diff is not long enough
        {
            ...
            + "The UNDEFINED Value: -1.0 is returned as result.", "Bin: " + getName() + " Method: double avgAvail ()",
    ...
    public desmoj.core.report.Reporter createDefaultReporter() {
        return new desmoj.core.advancedModellingFeatures.report.BinReporter(this);
    }
    ...
    /**
     * To reset the statistics of this bin. The number of available products at this
     * moment and the processes waiting in the queue are not changed. But all
     * statistic counters are reset. The <code>QueueBased</code> is also reset.
     */
    @Override
    public void reset() {
     ...
     * there are not enough products available or another consumer is first in the
     ...
     *
     * The order of process activation depends on the order of the internal queue,
     * which is based on the consumers processes' queueing priorities and (if
     * queueing priorities are equal, requiring a tie-breaker) by FIFO or a
     * different discipline as defined in the constructor.
     *
     * @return boolean : Is <code>true</code> if the specified number of units have
     *         been delivered successfully, <code>false</code> otherwise (i.e.
    ...
    public boolean retrieve(long n) {
        ...
        if (n <= 0) // if the process is taking nothing or less
        {
            ...
            + " out of a Bin. The attempted action is ignored!", "Bin: " + getName() + " Method: " + _where,
        ...
        if (!_passBy) {
            ...
            // in the q
            {
                ...
                if (currentlySendDebugNotes()) {
                    sendDebugNote("can not deliver " + n + " units for " + currentProcess.getQuotedName() + "<br>"
                    + "because there are only " + getAvail() + " units " + "right now.");
                }
     ...
     * always store products into the Bin and will never be inserted in a queue and
     * blocked, because the Bin has no capacity limit..
     *
    ...
    public void store(long n) {
        ...
        if (n <= 0) // trying to store nothing or less, don't fool me!
        {
            ...
            + " in a Bin. The attempted action is ignored!", "Bin: " + getName() + " Method: void store (long n)",
     ...
     * trying to take products, but it could not get any because there were not
     * enough products for it or another process was first in the queue to be
     * served. This method is called every time a producer has given new products or
     * a consumer in the waiting-queue is satisfied.
     */
     ...
     * @param where String : The method having called <code>checkProcess()</code> as
     *              a String.
     */
    protected boolean checkProcess(SimProcess p, String where) {
        if (p == null) // if p is a null pointer instead of a process
        {
            sendWarning("A non existing process is trying to use a Bin object. " + "The attempted action is ignored!",
                        "Bin: " + getName() + " Method: " + where, "The process is only a null pointer.",
        ...
        if (!isModelCompatible(p)) // if p is not modelcompatible
        {
            ...
            + "belong to this model. The attempted action is ignored!", "Bin: " + getName() + " Method: " + where,
     ...
     *
     * @param n long : Is positive when producers <code>store()</code> new products
     *          in the bin and negative when the Bin <code>deliver()</code>'s
     *          products to the consumer process.
     */
