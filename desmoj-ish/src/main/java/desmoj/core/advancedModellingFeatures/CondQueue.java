...
public class CondQueue<P extends SimProcess> extends desmoj.core.simulator.QueueBased {
    ...
    public desmoj.core.report.Reporter createDefaultReporter() {
        return new desmoj.core.advancedModellingFeatures.report.CondQueueReporter(this);
    }
    ...
    public void waitCancel(P p) {
        ...
        if (p == null) {
            sendWarning("Can not cancel waiting of SimProcess in Queue!",
                        "CondQueue : " + getName() + " Method:  void remove(P p)",
                        "The SimProcess 'p' given as parameter is a null reference!",
                        "Check to always have valid references when removing " + "processes");
            return; // no proper parameter
        }
        if (!_queue.contains(p)) {
            sendWarning("Can not cancel waiting of SimProcess in Queue!",
                        "CondQueue : " + getName() + " Method:  void remove(P p)",
                        "The SimProcess 'p' given as parameter is not enqueued in this " + "CondQueue!",
        ...
        if (currentlySendTraceNotes()) {
            sendTraceNote("cancels waiting of " + p.getQuotedName() + " in " + this.getQuotedName());
        }
    }
    ...
    public boolean waitUntil(Condition<P> cond) {
        ...
        {
            if (currentlySendTraceNotes()) {
                sendTraceNote("waits in '" + getName() + "'" + " until '" + cond.getName() + "' "); // send a traceNote
            }
