 ...
 * time.
 * <p>
 * Entities typically encapsulate all information about a model entity relevant
 * to the modeller. Events can manipulate these informations when the scheduled
 * point of simulation time is reached and thus change the state of the model.
 * When modelling different types of entities you need to derive different
 * classes from this superclass. Each carrying the specific information to
 * represent its counterpart in the system modelled. Thus a simulation of e.g. a
 * factory would require both machines and material to be subclasses of class
 * <code>Entity</code>. They can act on each other by scheduling themselves or
 * other Entities with the appropriate events. To use more than one entity of
 * one type, create multiple instances of the same <code>Entity</code> class.
 ...
 * has been instantiated, applied for inserting Entities into any kind of
 * Queues: The entity's priority determines it's position inside the queue on
 * entering it. Although within a model all attributes of an entity could be
 * made public it is advisable to support data hiding by providing methods for
 * accessing the internal attributes, as always in oo-design.
 *
 * @see Event
 * @see SimProcess
 * @see NamedObject
...
public abstract class Entity extends Schedulable {
    ...
    /**
     * Checks if the two entities have the same priority. Note that this is a static
     * method available through calling the entity's class i.e.
     * <code>Entity.isEqual(a,b)</code> where <code>a</code> and <code>b</code> are
     ...
     * entity's class i.e. <code>Entity.isLarger(a,b)</code> where a and b are valid
     * Entity objects.
     *
    ...
    /**
     * Checks if the first of the two entities has higher or same priority than the
     * second. Note that this is a static method available through calling the
     * entity's class i.e. <code>Entity.isLargerOrEqual(a,b)</code> where a and b
    ...
    /**
     * Checks if the first of the two entities have different priorities. Note that
     * this is a static method available through calling the entity's class i.e.
     * <code>Entity.isNotEqual(a,b)</code> where a and b are valid Entity objects.
     *
     ...
     * class i.e. <code>Entity.isSmaller(a,b)</code> where a and b are valid Entity
     * objects.
     *
     * @return boolean : Is <code>true</code> if <code>a</code> has a lower priority
     *         than <code>b</code>,<code>/false</code> otherwise
     * @param a Entity : First comparand entity
     * @param b Entity : Second comparand entity
     */
    ...
    /**
     * Checks if the first of the two entities has lower or same priority than the
     * second. Note that this is a static method available through calling the
     * entity's class i.e. <code>Entity.isSmallerOrEqual(a,b)</code> where a and b
     * are valid Entity objects.
     *
    ...
    /**
     * The <code>Queue</code>s or other <code>QueueBased</code> objects where this
     * Entity is queued.
     */
    ...
    /**
     * Returns a list of queues and other <code>QueueBased</code> objects where this
     * <code>Entity</code> is queued.
     *
     * @return List<QueueBased> : The <code>QueueBased</code>s containing this
     *         entity; may be empty if entity is not queued.
     */
    ...
    /**
     * Returns a list of events associated to this Entity object. If the Entity
     * object is not currently scheduled, an empty list will be returned. Remind
     * that all different Event classes can be included.
     *
     * @return List<EventAbstract> : The events associated to the entity
     */
    public List<EventAbstract> getScheduledEvents() {
        List<EventAbstract> list = new LinkedList<>();
    ...
    /**
     * Tests if this <code>Entity</code> queued in a at least one queue or other
     * <code>QueueBased</code>.
     *
     * @return boolean : Is <code>true</code> if this Entity is queued in at least
     *         one queue or other <code>QueueBased</code>, <code>false</code>
     *         otherwise.
     */
    ...
    /**
     * Tests if this Entity actually is a SimProcess. Although SimProcess have an
     * individual life-cycle, they can also be handled like entities and be
     * scheduled to be manipulated by an event.
     *
     * @return boolean : Is <code>true</code> if this Entity is an instance of class
     *         <code>SimProcess</code>,<code>false</code> otherwise
     */
    ...
    /**
     * Schedules this Entity to be manipulated by the given EventOf2Entities at the
     * current time. No preemption, i.e. a process calling this method will continue
     * until passivated or hold.
     *
     * @param who2 Entity : The second entity to be scheduled for the
     *             EventOf2Entities.
    ...
    public <E extends Entity> void schedule(E who2, EventOf2Entities<?, E> what) {
        ...
        if ((who2 == null)) {
            sendWarning("Can't schedule Entity! Command ignored.",
                        "Entity : " + getName()
                        + " Method: <E extends Entity> schedule(E who2, EventOf2Entities<?, E> what) {",
        ...
        if ((what == null)) {
            sendWarning("Can't schedule Entity! Command ignored.",
                        ...
                        "Be sure to have a valid EventOf2Entities reference for this event " + "to be scheduled with.");
            return; // no proper parameter
        }
        ...
        if (!isModelCompatible(what)) // Entity and Event are part of model
        {
            sendWarning("Can't schedule Entity! Command ignored",
                        "Entity : " + getName()
                        + " Method: <E extends Entity> schedule(E who2, EventOf2Entities<?, E> what) {",
                        "The EventOf2Entities to be scheduled with this Entity is not " + "modelcompatible.",
    ...
    /**
     * Schedules this Entity to be manipulated by the given EventOf2Entities at the
     * given point of time. Method returns with a warning message if either Entity
     * or Event are already scheduled in the event-list. No preemption, i.e. a
     * process calling this method will continue until passivated or hold.
     *
     * @param who2 Entity : The second entity to be scheduled for the
     *             EventOf2Entities.
     * @param what EventOf2Entities : The event to be scheduled
     * @param when TimeInstant : The point in simulation time this event is
     *             scheduled to happen.
     * @see SimClock
     */
    public <E extends Entity> void schedule(E who2, EventOf2Entities<?, E> what, TimeInstant when) {
        ...
        if ((who2 == null)) {
            sendWarning("Can't schedule Entity!",
                        "Entity : " + getName()
    ...
    /**
     * Schedules this Entity to be manipulated by the given EventOf2Entities at the
     * current time plus the given offset. No preemption, i.e. a process calling
     * this method will continue until passivated or hold.
     *
     * @param who2 Entity : The second entity to be scheduled for the
     *             EventOf2Entities.
     * @param what EventOf2Entities : The event to be scheduled
     * @param dt   TimeSpan : The offset to the current simulation time at which the
    ...
    /**
     * Schedules this Entity to be manipulated by the given EventOf3Entities at the
     * current time. No preemption, i.e. a process calling this method will continue
     * until passivated or hold.
     *
     * @param who2 Entity : The second entity to be scheduled for the
     *             EventOf3Entities.
     * @param who3 Entity : The third entity to be scheduled for the
     *             EventOf3Entities.
    ...
    public <E extends Entity, F extends Entity> void schedule(E who2, F who3, EventOf3Entities<?, E, F> what) {
        ...
        if ((who2 == null)) {
            sendWarning("Can't schedule Entity! Command ignored.", "Entity : " + getName()
        ...
        if ((what == null)) {
            ...
            + " Method: <E extends Entity, F extends Entity> schedule(E who2, F who3, EventOf3Entities<?, E, F> what)",
                        ...
                        "Be sure to have a valid Event reference for this event " + "to be scheduled with.");
            return; // no proper parameter
        }
        ...
        if (!isModelCompatible(what)) // Entity and Event are part of model
        {
            sendWarning("Can't schedule Entity! Command ignored", "Entity : " + getName()
            + " Method: <E extends Entity, F extends Entity> schedule(E who2, F who3, EventOf3Entities<?, E, F> what)",
                        "The EventOf3Entities to be scheduled with this Entity is not " + "modelcompatible.",
                        "Make sure to use compatible model components only.");
            return; // was already scheduled
        }
    ...
    /**
     * Schedules this Entity to be manipulated by the given EventOf3Entities at the
     * given point of time. No preemption, i.e. a process calling this method will
     ...
     *             EventOf3Entities.
     * @param what EventOf3Entities : The event to be scheduled
     * @param when TimeInstant : The point in simulation time the event is scheduled
     *             to happen.
     * @see SimClock
     */
    public <E extends Entity, F extends Entity> void schedule(E who2, F who3, EventOf3Entities<?, E, F> what,
        ...
        if ((who2 == null)) {
            sendWarning("Can't schedule Entity!", "Entity : " + getName()
        ...
        if ((what == null)) {
            ...
            + " Method: <E extends Entity,F extends Entity> schedule(E who2, F who3, EventOf3Entities<?, E, F> what, TimeInstant when) {",
                        "The EventOf3Entities given as parameter is a null reference.",
                        "Be sure to have a valid EventOf3Entities reference for this event " + "to be scheduled with.");
            return; // no proper parameter
        }
     ...
     * time. Method returns with a warning message if either Entity or Event are
     * already scheduled in the EventList. No preemption, i.e. a process calling
     * this method will continue until passivated or hold.
     *
    ...
    public void schedule(Event<?> what) {
        ...
        if ((what == null)) {
            sendWarning("Can't schedule Entity! Command ignored.",
                        "Entity : " + getName() + " Method: schedule(Event what)",
        ...
        if (!isModelCompatible(what)) // Entity and Event are part of model
        {
            sendWarning("Can't schedule Entity! Command ignored",
                        "Entity : " + getName() + " Method: schedule(Event what)",
                        "The Event to be scheduled with this Entity is not " + "modelcompatible.",
                        "Make sure to use compatible model components only.");
            return; // was already scheduled
        }
    ...
    /**
     * Schedules this Entity to be manipulated by the given Event at the given point
     * of time. Method returns with a warning message if either Entity or Event are
     * already scheduled in the event-list. No preemption, i.e. a process calling
     * this method will continue until passivated or hold.
     *
     * @param what Event : The Event that manipulates this Entity
     * @param when TimeInstant : The point in simulation time this event is
    ...
    public void schedule(Event<?> what, TimeInstant when) {
        if ((when == null)) {
            sendWarning("Can't schedule Entity!",
                        "Entity : " + getName() + " Method: schedule(Event what, TimeInstant when)",
                        "The simulation time given as parameter is a null reference.",
                        "Be sure to have a valid simulation time reference before " + "calling this method.");
            return; // no proper parameter
        }
    ...
    /**
     * Schedules this Entity to be manipulated by the given Event at the current
     * time plus the given offset. Method returns with a warning message if either
     * Entity or Event are already scheduled in the EventList. No preemption, i.e. a
     * process calling this method will continue until passivated or hold.
     *
     * @param what Event : The Event that manipulates this Entity
     * @param dt   TimeSpan : The offset to the current simulation time at which the
    ...
    public void schedule(Event<?> what, TimeSpan dt) {
        if ((dt == null)) {
            sendWarning("Can't schedule Entity!",
                        "Entity : " + getName() + " Method: schedule(Event what, TimeSpan dt)",
                        "The simulation time given as parameter is a null reference.",
                        "Be sure to have a valid simulation time reference before " + "calling this method.");
            return; // no proper parameter
        }
    ...
    /**
     * Schedules this Entity with the given Event to occur directly after the given
     * Schedulable that is already scheduled. Note that this event's point of
     * simulation time will be set to be the same as the Schedulable's time. Thus
     * the event will occur directly after the given Schedulable but the simulation
     * clock will not change. Will return with a warning message if the Schedulable
     * given as parameter is not scheduled. If there are multiple schedules for the
     * given Schedulable, the event will be scheduled after the last occurrence.
     *
    ...
    public void scheduleAfter(Schedulable after, Event<?> what) {
        ...
        if ((what == null)) {
            sendWarning("Can't schedule Entity! Command ignored.",
                        "Entity : " + getName() + " Method: scheduleAfter(Schedulable after, Event what)",
                        "The Event given as parameter is a null reference.",
                        "Be sure to have a valid Event reference before calling " + "this method.");
            return; // no proper parameter
        }
        ...
        if ((after == null)) {
            sendWarning("Can't schedule Entity! Command ignored.",
                        ...
                        "Be sure to have a valid Schedulable reference for this " + "Entity to be scheduled with.");
            return; // no proper parameter
        }
        ...
        if (currentlySendTraceNotes()) {
            if (this == currentEntity()) {
                sendTraceNote("schedules '" + what.getName() + "' of itself after '" + after.getName() + "' at "
                + after.getEventNotes().get(after.getEventNotes().size() - 1).getTime().toString());
            } else {
                sendTraceNote("schedules '" + what.getName() + "' of '" + getName() + "' after '" + after.getName()
                + "' at " + after.getEventNotes().get(after.getEventNotes().size() - 1).getTime().toString());
            }
        }
        ...
        // schedule Event
        getModel().getExperiment().getScheduler().scheduleAfter(after, this, what);
    ...
    /**
     * Schedules this Entity with the given EventOf2Entities to occur directly after
     * the given Schedulable that is already scheduled. Note that the event's point
     * of simulation time will be set to be the same as the Schedulable's time. Thus
     * the event will occur directly after the given Schedulable but the simulation
     * clock will not change. Will return with a warning message if the Schedulable
     * given as parameter is not scheduled. If there are multiple schedules for the
     * given Schedulable, the event will be scheduled after the last occurrence.
     ...
     *              EventOf2Entities.
     * @param what  EventOf2Entities : The event to be scheduled
     * @param after Schedulable : The Schedulable the event should be scheduled
     *              after
     */
    public <E extends Entity> void scheduleAfter(Schedulable after, EventOf2Entities<?, E> what, E who2) {
        ...
        if ((what == null)) {
            ...
            + " Method: <E extends Entity> scheduleAfter(Schedulable after, EventOf2Entities<?, E> what, E who2)",
                        "The EventOf2Entities given as parameter is a null reference.",
                        "Be sure to have a valid EventOf2Entities reference before calling " + "this method.");
            return; // no proper parameter
        }
        ...
        if ((who2 == null)) {
            ...
            + " Method: <E extends Entity> scheduleAfter(Schedulable after, EventOf2Entities<?, E> what, E who2)",
                        "The Entity 'who2' given as parameter is a null reference.",
                        "Be sure to have a valid Entity reference for this " + "Entity to be scheduled with.");
            return; // no proper parameter
        }
        ...
        // schedule Event
        getModel().getExperiment().getScheduler().scheduleAfter(after, this, who2, what);
    ...
    /**
     * Schedules this Entity with the given EventOf3Entities to occur directly after
     * the given Schedulable that is already scheduled. Note that the event's point
     * of simulation time will be set to be the same as the Schedulable's time. Thus
     * the event will occur directly after the given Schedulable but the simulation
     * clock will not change. Will return with a warning message if the Schedulable
    ...
    public <E extends Entity, F extends Entity> void scheduleAfter(Schedulable after, EventOf3Entities<?, E, F> what,
        ...
        if ((what == null)) {
            ...
            + " Method: <E extends Entity, F extends Entity> scheduleAfter(Schedulable after, EventOf3Entities<?, E, F> what, E who2, F who3)",
                        "The EventOf3Entities given as parameter is a null reference.",
                        "Be sure to have a valid EventOf3Entities reference before calling " + "this method.");
            return; // no proper parameter
        }
        ...
        // schedule Event
        getModel().getExperiment().getScheduler().scheduleAfter(after, this, who2, who3, what);
    ...
    /**
     * Schedules this Entity with the given Event to occur directly before the given
     * Schedulable that is scheduled. Note that this event's point of simulation
     * time will be set to be the same as the Schedulable's time. Thus the event
     * will occur directly before the given Schedulable but the simulation clock
     * will not change. Issues a warning message if the Schedulable given is not
     * scheduled. If there are multiple schedules for the given Schedulable, the
    ...
    public void scheduleBefore(Schedulable before, Event<?> what) {
        ...
        if ((what == null)) {
            sendWarning("Can't schedule Entity! Command ignored.",
                        "Entity : " + getName() + " Method: scheduleBefore(Schedulable before, Event what)",
                        "The Event given as parameter is a null reference.",
                        "Be sure to have a valid Event reference before calling " + "this method.");
            return; // no proper parameter
        }
        ...
        if (!isModelCompatible(what)) {
            sendWarning("Can't schedule Entity! Command ignored",
                        "Entity : " + getName() + " Method: scheduleBefore(Schedulable before, Event what)",
                        "The Event to be scheduled with thisEntity is not " + "modelcompatible.",
                        "Make sure to use compatible model components only.");
            return; // was already scheduled
        }
        ...
        // schedule Event
        getModel().getExperiment().getScheduler().scheduleBefore(before, this, what);
    ...
    /**
     * Schedules this Entity with the given EventOf2Entities to occur directly
     * before the given Schedulable that is scheduled. Note that the event's point
     * of simulation time will be set to be the same as the Schedulable's time. Thus
     * the event will occur directly before the given Schedulable but the simulation
     * clock will not change. Issues a warning message if the Schedulable given is
     * not scheduled. If there are multiple schedules for the given Schedulable, the
     * event will be scheduled before the first occurrence.
     *
     * @param who2   Entity : The second entity to be scheduled for the
     *               EventOf2Entities.
    ...
    public <E extends Entity> void scheduleBefore(Schedulable before, EventOf2Entities<?, E> what, E who2) {
        ...
        // schedule Event
        getModel().getExperiment().getScheduler().scheduleBefore(before, this, who2, what);
    ...
    /**
     * Schedules this Entity with the given EventOf3Entities to occur directly
     * before the given Schedulable that is scheduled. Note that the event's point
     * of simulation time will be set to be the same as the Schedulable's time. Thus
     * the event will occur directly before the given Schedulable but the simulation
     * clock will not change. Issues a warning message if the Schedulable given is
    ...
    public <E extends Entity, F extends Entity> void scheduleBefore(Schedulable before, EventOf3Entities<?, E, F> what,
        ...
        // schedule Event
        getModel().getExperiment().getScheduler().scheduleBefore(before, this, who2, who3, what);
    ...
    /**
     * Schedules this entity with an event to be executed immediately, preempting
     * the process lifecycle executed at the moment.
     *
     * @param who2 Entity : The second entity to be scheduled for the
     *             EventOf2Entities.
     * @param what EventOf2Entities : The event to be scheduled
     ...
     * priority of each entity (unless assigned otherwise) is zero. Negative
     * priorities are lower, positive priorities are higher. All values should be
     * inside the range defined by Java's integral <code>integer</code> data type
     * [-2147483648, +2147483647].
     *
     * An entity's queuing priority can be used by the modeller to determine how the
     * entity is treated by queues, though how precisely a queue will use the
     * priority to determine sort order is up to it's queuing strategy:
     * <ul>
     * <li><code>QueueBased.FIFO</code> sorts entities by their queuing priority,
     * highest priority first. Entities with the same priority are enqueued based on
     * &quot;first in, first out&quot;.</li>
     * <li><code>QueueBased.LIFO</code> also sorts entities by their priority,
     * highest priority first. However, entities with the same priority are enqueued
     * based on &quot;last in, first out&quot;.</li>
     * <li><code>QueueBased.Random</code> assigns a random position to each entity
     * entering the queue, disregarding priority.</li>
     * </ul>
     * Of course, the modeller is free to use the queuing priority to determine how
     * entities are processed by the components he implements himself, whether they
     * are queues or not.
     *
    ...
    /**
     * Creates and returns a copy of this entity. Note that subclasses have to
    ...
    protected Entity clone() throws CloneNotSupportedException {
        ...
        c._myQueues = new ArrayList<>();
        c._identNumber = this.getModel().linkWithIdentNumber(c);
        return c;
    }

    /**
     * Utility method to generate trace output for scheduling this event (internal
     * use only).
     *
     * @param Event  the event to be scheduled
     * @param who1   the second entity scheduled with the event (or
     *               <code>null</code> if not applicable)
     * @param who2   the third entity scheduled with the event (or <code>null</code>
     *               if not applicable)
     * @param after  the Schedulable after which the event is scheduled (or
     *               <code>null</code> if not applicable)
     * @param before the Schedulable before which the event is scheduled (or
     *               <code>null</code> if not applicable)
     * @param at     the TimeInstant at which the event is scheduled
     * @param remark optional further comments (or <code>null</code> if not
     *               applicable)
     */
    ...
    /**
     * Informs this <code>Entity</code> to be queued in a given
     * <code>QueueBased</code>.
     *
     * @param q QueueBased : The <code>QueueBased</code> where this entity is now
     *          queued.
     */
    ...
    /**
     * Informs this <code>Entity</code> to be no longer queued in a given
     * <code>QueueBased</code>.
     *
     * @param q Queue<?> : The <code>QueueBased</code> where this entity is no
     *          longer queued.
     */
