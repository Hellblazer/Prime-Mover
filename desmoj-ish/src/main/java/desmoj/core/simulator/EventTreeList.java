...
public class EventTreeList extends EventList {
    ...
    void insert(EventNote newNote) {
        ...
        if (isEmpty()) { // no worry about order if it's first
            ...
            do {
                ...
                if (TimeInstant.isBefore(((EventNote) eTreeList.get(index)).getTime(), refTime) || (TimeInstant.isEqual(
                ((EventNote) eTreeList.get(index)).getTime(), refTime) && ((EventNote) eTreeList.get(
                index)).getPriority() >= refPrio)) {
    ...
    void insertAfter(EventNote where, EventNote newNote) {
        ...
        if (i < 0) { // negative index means, that where is not contained
            ...
            new ErrorMessage(mBuffer, "Can not insert new event-note after given EventNote! " + "Simulation aborted",
                             ...
                             + "in the event tree list.", "This is a fatal error. Contact DESMOJ support",
                             newNote.getTime()));
    ...
    void insertBefore(EventNote where, EventNote newNote) {
        ...
        if (i < 0) {
            ...
            throw new SimAbortedException(
            new ErrorMessage(mBuffer, "Can not insert new event-note before given EventNote! " + "Simulation aborted",
                             "Internal DESMO-J class : EventTreeList Method : "
