...
/**
 * Provides the abstract super class for user defined events to change an
 * entity's internal state. The state of a discrete model is changed by events
 * that occur at distinct points of simulation time.
 * <p>
...
public abstract class EventAbstract extends Schedulable {
    ...
    /**
     * The realTime deadline for this event in nanoseconds. In case of a real-time
     * execution (i. e. the execution speed rate is set to a positive value) the
     * Scheduler will produce a warning message if a deadline is missed.
     */
    ...
    /**
     * Returns the event's scheduling priority. The scheduling priority is used to
     * determine which event to execute first if two or more events are scheduled at
     * the same instant. The default priority is zero. Higher priorities are
     * positive, lower priorities negative.
     *
     ...
     *
     * An event's scheduling priority it used to determine which event is executed
     * first if scheduled for the same time instant. Should the priority be the
     * same, event execution depends on the <code>EventList</code> in use, e.g.
     * scheduled first is executed first (<code>EventTreeList</code>) or random
     ...
     * use only).
     *
     * @param who1   the first entity scheduled with this event (or
     *               <code>null</code> if not applicable)
     * @param who2   the second entity scheduled with this event (or
     *               <code>null</code> if not applicable)
     * @param who3   the third entity scheduled with this event (or
     *               <code>null</code> if not applicable)
     * @param after  the Schedulable after which this event is scheduled (or
     *               <code>null</code> if not applicable)
     * @param before the Schedulable before which this event is scheduled (or
     *               <code>null</code> if not applicable)
    ...
    protected void generateTraceForScheduling(Entity who1, Entity who2, Entity who3, Schedulable after,
        ...
        if (currentlySendTraceNotes()) {
            ...
            if (who1 != null) {
                String who1alias = (who1 == currentEntity() && who2 == null && who3 == null ? "itself" : "'"
                + who1.getName() + "'");
