...
/**
 * A <code>ComplexSimProcess</code> is a <code>SimProcess</code> which serves as
 * a container for other <code>SimProcess</code> es. A
...
public abstract class ComplexSimProcess extends SimProcess {
    ...
    /**
     * The <code>Vector</code> holding all the SimProcesses which are contained in
     ...
     * @param showInTrace boolean : Flag for showing trace messages of this
     *                    ComplexSimProcess in trace-files. Set it to
     *                    <code>true</code> if ComplexSimProcess should show up in
     *                    trace. Set it to <code>false</code> if ComplexSimProcess
    ...
    /**
     * Adds a SimProcess as a component to this ComplexSimProcess. The Sim-process
     ...
     */
    public void addComponent(SimProcess compnt) {
        ...
        try {
            ...
            if (compnt == null) // if compnt is a null pointer instead of a process
            {
                ...
                + "ComplexSimProcess. The attempted action is ignored!",
                            "ComplexSimProcess: " + getName() + " Method: synchronized void"
                            + " addComponent(SimProcess compnt)", "The given SimProcess is only a null pointer.",
                            "Make sure that only real SimProcesses are added to a  " + "ComplexSimProcess.");
            ...
            {
                sendWarning("The process trying to be added to a ComplexSimProcess does "
                + "not belong to this model. The attempted action is ignored!",
                            "ComplexSimProcess: " + getName() + " Method: synchronized void"
                            + " addComponent(SimProcess compnt)",
                            "The given process belongs to model " + compnt.getModel().getQuotedName()
                            + " and therefore is not modelcompatible.",
                            "Make sure that the processes to be added to a ComplexSimProcess belong "
                            + "to the same model or overwrite the method " + "<code>isModelCompatible()</code>");
            ...
            if (_components.contains(compnt)) {
                sendWarning("Attempt to add a process to a ComplexSimProcess twice. "
                + "The attempted action is ignored!",
                            "ComplexSimProcess: " + getName() + " Method: synchronized void"
                            + " addComponent(SimProcess compnt)",
                            "The given SimProcess is already a component of this " + "ComplexSimProcess.",
                            "Make sure the SimProcess is not contained in the " + "ComplexSimProcess already.");
                return;
            }
            ...
            if (compnt.isScheduled()) {
                sendWarning("The SimProcess added to a ComplexSimProcess is scheduled! "
                + "The scheduled activation of the SimProcess will be " + "cancelled.",
                            "ComplexSimProcess: " + getName() + " Method: synchronized void"
                            + " addComponent(SimProcess compnt)",
                            "A SimProcess added to a ComplexSimProcess is giving up its "
                            + "own lifeCycle and therefore should not be scheduled anymore."
                            + " It will be carried on by the lifeCycle of the " + "ComplexSimProcess.",
            ...
            if (currentlySendTraceNotes()) {
                sendTraceNote("adds " + compnt.getQuotedName() + " as a component to " + this.getQuotedName());
            }

            // debug output
            if (currentlySendDebugNotes()) {
                sendDebugNote("adds " + compnt.getQuotedName() + " to its components." + "it now looks like <br>"
                + this.toString());
            }
     ...
     * @param elem desmoj.SimProcess : The SimProcess which might be an element of
     *             this ComplexSimPorcess already.
     ...
     * implement the specific behaviour of this process. This method starts after a
     * <code>ComplexSimProcess</code> has been created and activated by the
     * scheduler. As long as this <code>ComplexSimProcess</code> is active all its
     * contained Simprocesses are passive. That means they are blocked and can not
     ...
     * ComplexSimProcess. This will be done for all ComplexSimPorcesses recursively,
     * until all simple SimProcesses are removed. The SimProcesses being removed
     * from the ComplexSimProcess will be activated after the current SimProcess so
     * they can follow their own lifeCycle again. Of course, this does only make
     * sense for SimProcesses which are not terminated already.
     */
    ...
    public void removeComponent(SimProcess elem) {
        ...
        try {
            ...
            if (elem == null) // if elem is a null pointer instead of a process
            {
                ...
                + "ComplexSimProcess. The attempted action is ignored!",
                            "ComplexSimProcess: " + getName() + " Method: synchronized void"
                            + "removeComponent(SimProcess elem)", "The given SimProcess is only a null pointer.",
                            "Make sure to remove only SimProcesses that are contained in this " + "ComplexSimProcess.");
            ...
            if (!_components.contains(elem)) {
                sendWarning("Attempt to remove a process which is not an element of this "
                + "ComplexSimProcess. The attempted action is ignored!",
                            "ComplexSimProcess: " + getName() + " Method: synchronized void"
                            + "removeComponent(SimProcess elem)",
                            "The given SimProcess is not a component of this " + "ComplexSimProcess.",
                            "Make sure to remove only SimProcesses that are contained in " + "this ComplexSimProcess.");
                return;
            }
            ...
            // trace output
            if (currentlySendTraceNotes()) {
                sendTraceNote("removes " + elem.getQuotedName() + " from " + this.getQuotedName());
            }

            // debug output
            if (currentlySendDebugNotes()) {
                sendDebugNote("removes " + elem.getQuotedName() + " from its elements." + "it now looks like <br>"
                + this.toString());
            }
        } finally {
            _lock.unlock();
    ...
    public String toString() {
        ...
        if (!hasComponents()) {
            ...
            // loop through all elements
            for (Enumeration<?> e = getComponents(); e.hasMoreElements();) {
                stringOfElems.append(((SimProcess) e.nextElement()).getQuotedName());
