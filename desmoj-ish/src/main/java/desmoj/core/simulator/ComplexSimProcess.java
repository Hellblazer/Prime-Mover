...
public abstract class ComplexSimProcess extends SimProcess {
     ...
     *                    trace. Set it to <code>false</code> if ComplexSimProcess should not be shown in trace.
     */
    ...
    public void addComponent(SimProcess compnt) {
        ...
        try {
            ...
            {
                sendWarning(
                "Attempt to add a non existing process to a " + "ComplexSimProcess. The attempted action is ignored!",
                "ComplexSimProcess: " + getName() + " Method: synchronized void" + " addComponent(SimProcess compnt)",
            ...
            if (_components.contains(compnt)) {
                sendWarning(
                "Attempt to add a process to a ComplexSimProcess twice. " + "The attempted action is ignored!",
                "ComplexSimProcess: " + getName() + " Method: synchronized void" + " addComponent(SimProcess compnt)",
            ...
            if (currentlySendDebugNotes()) {
                sendDebugNote(
                "adds " + compnt.getQuotedName() + " to its components." + "it now looks like <br>" + this.toString());
            }
     ...
     * ComplexSimProcess, as determined by the <code>equals()</code> method; <code>false</code> otherwise.
     ...
     * process. This method starts after a
    ...
    public void removeComponent(SimProcess elem) {
        ...
        try {
            ...
            if (currentlySendDebugNotes()) {
                sendDebugNote(
                "removes " + elem.getQuotedName() + " from its elements." + "it now looks like <br>" + this.toString());
            }
        } finally {
            _lock.unlock();
        }
