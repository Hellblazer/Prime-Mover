...
public abstract class Event<E extends Entity> extends EventAbstract {
    ...
    public void schedule(E who) {
        ...
        if ((who == null)) {
            sendWarning("Can't schedule Event!", "Event : " + getName() + " Method: schedule(Entity who)",
        ...
        if (!isModelCompatible(who)) {
            sendWarning("Can't schedule Event! Command ignored",
                        "Entity : " + getName() + " Method: schedule(Entity who)",
                        "The Entity to be scheduled with this event is not " + "modelcompatible.",
    ...
    public void schedule(E who, NumericalDist<?> dist) {
        ...
        if ((dist == null)) {
            sendWarning("Can't schedule Event!",
                        "Event : " + getName() + " Method: schedule(Entity who, NumericalDist<?> dist)",
        ...
        if (!isModelCompatible(who)) {
            sendWarning("Can't schedule Event! Command ignored",
                        "Entity : " + getName() + " Method: schedule(Entity who, NumericalDist<?> dist)",
                        "The Entity to be scheduled with this event is not " + "modelcompatible.",
                        "Make sure to use compatible model components only.");
            return; // is not compatible
        }
    ...
    public void schedule(E who, TimeInstant instant) {
        ...
        if ((instant == null)) {
            sendWarning("Can't schedule Event!",
                        "Event : " + getName() + " Method: schedule(Entity who, TimeInstant instant)",
        ...
        if (!isModelCompatible(who)) {
            sendWarning("Can't schedule Event! Command ignored",
                        "Entity : " + getName() + " Method: schedule(Entity who, TimeInstant instant)",
                        "The Entity to be scheduled with this event is not " + "modelcompatible.",
                        "Make sure to use compatible model components only.");
            return; // is not compatible
        }
    ...
    public void schedule(E who, TimeSpan dt) {
        if ((dt == null)) {
            sendWarning("Can't schedule Event!", "Event : " + getName() + " Method: schedule(Entity who, TimeSpan dt)",
        ...
        if (!isModelCompatible(who)) {
            sendWarning("Can't schedule Event! Command ignored",
                        "Entity : " + getName() + " Method: schedule(Entity who, TimeSpan dt)",
                        "The Entity to be scheduled with this event is not " + "modelcompatible.",
                        "Make sure to use compatible model components only.");
            return; // is not compatible
        }
    ...
    public void scheduleAfter(Schedulable after, E who) {
        ...
        if (who == null) {
            sendWarning("Can't schedule Event! Command ignored.",
                        "Event : " + getName() + " Method: scheduleAfter(Schedulable after, " + "Entity who)",
        ...
        if (who.isScheduled()) {
            sendWarning("Can't schedule Event! Command ignored.",
                        "Entity : " + getName() + " Method: scheduleAfter(Schedulable after, " + "Entity who)",
                        "The Entity '" + who.getName() + "'to be scheduled with this " + "Event is already scheduled.",
    ...
    public void scheduleBefore(Schedulable before, E who) {
        ...
        if ((who == null)) {
            sendWarning("Can't schedule Event! Command ignored.",
                        "Event : " + getName() + " Method: scheduleBefore(Schedulable before, " + "Entity who)",
                        "The Entity given as parameter is a null reference.",
                        "Be sure to have a valid Entity reference before calling " + "this method.");
            return; // no proper parameter
        }
        ...
        if (!before.isScheduled()) {
            sendWarning("Can't schedule Event! Command ignored.",
                        "Event : " + getName() + " Method: scheduleBefore(Schedulable after, " + "Entity who)",
        ...
        if (!isModelCompatible(who)) {
            sendWarning("Can't schedule Event! Command ignored",
                        "Event : " + getName() + " Method: scheduleBeforer(Schedulable before, " + "Entity who)",
                        "The Entity to be scheduled with this event is not " + "modelcompatible.",
                        "Make sure to use compatible model components only.");
            return; // is not compatible
        }
     ...
     * @
     */
    public void schedulePreempt(E who) {
        ...
        if (getModel().getExperiment().getScheduler().getCurrentSimProcess() == null) {
            sendWarning("Can't preempt current SimProcess! " + "Command ignored.",
                        "Event : " + getName() + " Method: schedulePreempt(E who)", "No current process.",
                        "Call this method during process execution only.");
            return; // preemption of currentprocess only
        }

        if ((who == null)) {
            sendWarning("Can't schedule Event!", "Event : " + getName() + " Method: schedulePreempt(Entity who)",
                        "The Entity given as parameter is a null reference.",
                        "Be sure to have a valid Entity reference for this event to " + "be scheduled with.");
            return; // no proper parameter
        }
        ...
        if (!isModelCompatible(who)) {
            sendWarning("Can't schedule Event! Command ignored",
                        "Entity : " + getName() + " Method: schedulePreempt(Entity who)",
                        "The Entity to be scheduled with this event is not " + "modelcompatible.",
                        "Make sure to use compatible model components only.");
            return; // is not compatible
        }
