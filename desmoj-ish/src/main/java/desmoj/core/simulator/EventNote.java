 ...
 * <Code>Schedulable</Code>, event or process. Since <Code>EventNote</Code> are
 * tightly coupled with the <Code>EventList</Code>, a change of the
 * <Code>EventList</Code> might alo force the <Code>EventNote</Code> to contain
 * other information especially concerning the data structures used in the
 * <Code>EventList</Code> as well. To adopt the <Code>Event notes</Code> extend
 * them to carry the specific data and methods you need for a possible change of
 * the <Code>EventList</Code>. The implementation here includes data and methods
 * for event information only, since the default implementation of class
 * <code>EventVector</code> needs no special information stored inside the
 * <Code>Event notes</Code>.
 *
...
public class EventNote implements Comparable<EventNote> {
     ...
     * <code>RandomizingEventTreeList</code> will not insert another event-note (but
     * not explicitly scheduled using <code>scheduleBefore()</code> or
     * <code>scheduelAfter()</code>) between this event-note and its predecessor
     ...
     * <code>null</code> in case of an external event associated to this EventNote.
     */
    ...
    /**
     * The point of simulation time associated to an event and an entity. Must never
     * be <code>null</code> since changes in the state of a model always happen to a
     * certain discrete point of time.
     */
     ...
     * creation time. Note that all relevant associations of the given Schedulables
     * towards this event-note are built in this constructor ensuring that each
     * scheduled Schedulable has access to its associated EventNote.
     *
     * @param who1         Entity : The first entity that is scheduled to be change
     *                     during the course of the event or the SimProcess (or
     *                     null, if such an entity is not defined)
     * @param who2         Entity : The second entity that is scheduled to be change
     *                     during the course of the event or the SimProcess (or
     *                     null, if such an entity is not defined)
     * @param who3         Entity : The third entity that is scheduled to be change
     *                     during the course of the event or the SimProcess (or
     *                     null, if such an entity is not defined)
     * @param what         Event : The type of Event that scheduled to happen to the
     *                     Entity or entities
     * @param when         TimeInstant : The point of time that this event-note is
     *                     supposed to be processed by the scheduling mechanism
     * @param howImportant int : The scheduling priority of this event note. Higher
     *                     means more important.
     * @param source       Schedulable : The Schedulable responsible for creating
     *                     this event note
     */
    ...
    /**
     * Compares the given EventNote to this event-note. This method implements the
     * Comparable<EventNote> Interface
     *
     ...
     *         event-note is before, at the same time, or after the given EventNote.
     */
    @Override
    ...
    /**
     * Copies the event-note. WARNING: No additional reference is set to linked
     * Entity.
     *
     * @param source Schedulable : The Schedulable responsible for copying this
     *               event note
     *
    ...
    public boolean equals(Object object) {
        ...
        return _myTimeInstant.equals(note._myTimeInstant) && equalsWithNull(_myEvent, note._myEvent) &&
               equalsWithNull(_myEntity1, note._myEntity1) && equalsWithNull(_myEntity3, note._myEntity2) &&
               equalsWithNull(_myEntity2, note._myEntity3) && _myPriority == note._myPriority;
    }
    ...
    @Override
    public int hashCode() {
        return (_myEntity1 == null ? 13 : _myEntity1.hashCode()) ^ (_myEntity2 == null ? 983 : _myEntity2.hashCode())
               ^ (_myEntity3 == null ? 33637 : _myEntity3.hashCode())
               ^ (_myEvent == null ? 7288583 : _myEvent.hashCode()) ^ _myTimeInstant.hashCode();
    }

    /**
     * Returns a string representing the elements bundled in this event-note. It
     * calls the <code>toString()</code> methods of every element putting each in
     * brackets containing one or two letters to indicate the type of element.
     * <p>
     * <ul>
    ...
    /**
     * Tests if this event-note is connected to its predecessor. If a connection
     * exists (true), <code>RandomizingEventVector</code> or
     * <code>RandomizingEventTreeList</code> will not insert another event-note
     * scheduled for the same instant (but not explicity scheduled using
     * <code>scheduleBefore()</code> or <code>scheduelAfter()</code>) between this
     * event-note and its predecessor scheduled for the same instant. This is a
     * package visibility method for internal framework use only.
     *
     * @return boolean : Is <code>true</code> if a connection to the predecessor
     ...
     * <code>RandomizingEventVector</code> or <code>RandomizingEventTreeList</code>
     * will not insert another EventNote (but not explicity scheduled using
     * <code>scheduleBefore()</code> or <code>scheduelAfter()</code>) between this
     * event-note and its predecessor scheduled for the same instant or removes such
     * a connection (false). This is a package visibility method for internal
     * framework use only.
     *
     * @param isConnected boolean : establishes (true) or removes (false) a
     *                    connection between this event-note and its predecessor.
     */
    ...
    /**
     * Associates this event-note with the given event. This is a package visibility
    ...
    /**
     * Sets the priority for this event-note to the time given as parameter. This
     * method is to be used by the scheduler to correct the point of time of an
     * event-note after inserting it relative to some other EventNote to preserve
     * the temporal+priority order of the event-list. This is a package visibility
     * method for internal framework use only.
     *
     ...
     * This method is to be used by the scheduler to correct the point of time of an
     * event-note after inserting it relative to some other EventNote to preserve
     * the temporal order of the event-list. This is a package visibility method for
     * internal framework use only.
     *
     * @param time TimeInstant : the new point of simulation time this event-note is
     *             associated with.
     */
