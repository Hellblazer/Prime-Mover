            ...
            while (true) {
                ...
                while (!_realTimeEventQueue.isEmpty()) {
                    ...
                    if (weightedRealTimeEventNanos < 0) {
                        ...
                        if (TimeInstant.isAfterOrEqual(realTimeNanosEquivalent, presentTime())) {
                            // the event can be scheduled correctly
                            currentRealTimeEvent.schedule(realTimeNanosEquivalent);
                        } else {
...
//		_currentNote.setTime(null);
        ...
        if (_currentEvent != null) { // event-oriented
            ...
            if (!_currentProcess.isTerminated()) {
                ...
                myExperiment.sendWarning("Can not activate scheduled SimProcess! " + "Command ignored.",
                                         "Scheduler of experiment " + myExperiment.getName()
                                         + " Method processNextEventNote() " + "SimProcess : "
        ...
        if (_executionSpeedRate > 0) {
            ...
            if (_currentEvent != null) {
                ...
                if (deadLine > 0 && deadLine < deviationInNanoseconds) {
                    ...
                    myExperiment.sendWarning("The real-time deadline of this event was missed!",
                                             "Scheduler of experiment " + myExperiment.getName()
                                             + " Method processNextEventNote() " + "Event : " + _currentEvent.getName(),
                                             "The real-time deadline of this event was " + deadLine
                                             + " nanoseconds. It was missed by " + (deviationInNanoseconds - deadLine)
                ...
                if (deadLine > 0 && deadLine < deviationInNanoseconds) {
                    // the real-time deadline was missed
                    myExperiment.sendWarning("The real-time deadline of this SimProcess was missed!",
                                             "Scheduler of experiment " + myExperiment.getName()
                                             + " Method processNextEventNote() " + "SimProcess : "
                                             + _currentProcess.getName(),
                                             "The real-time deadline of this SimProcess was " + deadLine
                                             + " nanoseconds. It was missed by " + (deviationInNanoseconds - deadLine)
                                             + " nanoseconds.", "Check if the deadline can be met.");
                }
            }
    ...
    protected void reScheduleNoPreempt(Schedulable who, TimeInstant time) {
        ...
        if (time == null) {
            myExperiment.sendWarning("Can't reschedule Schedulable at given time! " + "Command ignored.",
                                     ...
                                     "The simulation time reference passed as parameter is a " + "null reference.",
                                     "Always make sure to use valid references only.");
            return; // time missing
        }
        ...
        if (who == null) {
            myExperiment.sendWarning("Can't reschedule Schedulable at given time! " + "Command ignored.",
                                     "Scheduler : " + getName()
                                     + " Method: reSchedule(Schedulable who, TimeInstant time)",
                                     "The Schedulable reference passed as parameter is a " + "null reference.",
                                     "Always make sure to use valid references only.");
        ...
        if (!who.isScheduled()) {
            myExperiment.sendWarning("Can't reschedule Schedulable at given time! " + "Command ignored.",
                                     "Scheduler : " + getName()
                                     + " Method: reSchedule(Schedulable who, TimeInstant time)",
                                     "The Schedulable given is not scheduled, thus can not be " + "rescheduled.",
                                     "To reschedule a Schedulable, it must already be scheduled. "
                                     + "You can check that by calling the Schedulable's " + "isScheduled() method");
            return;
    ...
    protected void reScheduleNoPreempt(Schedulable who, TimeSpan dt) {
        ...
        if (dt == null) {
            myExperiment.sendWarning("Can't reschedule Schedulable at given time! " + "Command ignored.",
                                     "Scheduler : " + getName() + " Method: reSchedule(Schedulable who, TimeSpan dt)",
                                     "The simulation time reference passed as parameter is a " + "null reference.",
                                     "Always make sure to use valid references only.");
            return; // time missing
        }
    ...
    protected void scheduleAfter(Schedulable after, Entity who, EventAbstract what) {
        ...
        if (after == null) {
            myExperiment.sendWarning("Can't schedule after Schedulable! " + "Command ignored.",
                                     "Scheduler : " + getName() + " Method: scheduleAfter(Schedulable after "
        ...
        if ((who == null) && (what == null)) {
            myExperiment.sendWarning("Can't schedule Entity and Event! " + "Command ignored.",
                                     "Scheduler : " + getName() + " Method: scheduleAfter(Schedulable after "
                                     + ", Entity who, EventAbstract what)",
                                     "The Event and Entity references passed are both " + "null references.",
        ...
        if ((who == null) && !(what instanceof ExternalEvent)) {
            myExperiment.sendWarning("Can't schedule Event! Command ignored.",
                                     "Scheduler : " + getName() + " Method: scheduleAfter(Schedulabe "
                                     ...
                                     "If no valid Entity is given, the event must be of type " + "external event.");
    ...
    protected void scheduleNoPreempt(Entity who1, Entity who2, Entity who3, EventOf3Entities<?, ?, ?> what,
        ...
        if (when == null) {
            myExperiment.sendWarning("Can't schedule Entity and Event at given " + "time! Command ignored.",
                                     "Scheduler : " + getName()
        ...
        if ((who1 == null) && (what == null)) {
            myExperiment.sendWarning("Can't schedule Entity and Event! " + "Command ignored.",
                                     ...
                                     + " Method: schedule(Entity who1, Entity who2, Entity who3, "
                                     + "EventOf3Entities what, TimeInstant when)",
                                     "The first Event and Entity references passed are both " + "null references.",
                                     "Either Event or Entity references must be valid.");
            return; // no real parameters here anyway
        }
        ...
        if ((who2 == null) && (what == null)) {
            myExperiment.sendWarning("Can't schedule Entity and Event! " + "Command ignored.",
                                     ...
                                     + " Method: schedule(Entity who1, Entity who2, Entity who3, "
                                     + "EventOf3Entities what, TimeInstant when)",
                                     "The EEvent and second entity references passed are both " + "null references.",
        ...
        if ((who3 == null) && (what == null)) {
            myExperiment.sendWarning("Can't schedule Entity and Event! " + "Command ignored.",
                                     ...
                                     + " Method: schedule(Entity who1, Entity who2, Entity who3, "
                                     + "EventOf3Entities what, TimeInstant when)",
                                     "The event and third entity references passed are both " + "null references.",
    ...
    protected void scheduleNoPreempt(Entity who1, Entity who2, Entity who3, EventOf3Entities<?, ?, ?> what,
        ...
        if ((who1 == null) && (what == null)) {
            myExperiment.sendWarning("Can't schedule Entity and Event! " + "Command ignored.",
                                     ...
                                     + " Method: schedule(Entity who1, Entity who2, Entity who3, "
                                     + "EventOf3Entities what, TimeSpan dt)",
                                     "The event and first entity references passed are both " + "null references.",
        ...
        if ((who2 == null) && (what == null)) {
            myExperiment.sendWarning("Can't schedule Entity and Event! " + "Command ignored.",
                                     ...
                                     + " Method: schedule(Entity who1, Entity who2, Entity who3, "
                                     + "EventOf3Entities what, TimeSpan dt)",
                                     "The event and second entity references passed are both " + "null references.",
                                     "Either Event or second entity references must be valid.");
            return; // no real parameters here anyway
        }
    ...
    protected void scheduleNoPreempt(Entity who1, Entity who2, EventOf2Entities<?, ?> what, TimeInstant when) {
        ...
        if (when == null) {
            myExperiment.sendWarning("Can't schedule Entity and Event at given " + "time! Command ignored.",
                                     "Scheduler : " + getName() + " Method: schedule(Entity who1, Entity who2, "
    ...
    protected void scheduleNoPreempt(Entity who, EventAbstract what, TimeInstant when) {
        ...
        if (TimeInstant.isBefore(when, this.presentTime())) {
            myExperiment.sendWarning("Can't reschedule Schedulable at given time! " + "Command ignored.",
                                     "Scheduler : " + getName() + " Method: schedule(Entity who, "
                                     + "EventAbstract what, TimeInstant when)", "The instant given is in the past.",
                                     "To schedule a Schedulable, use a TimeInstant no earlier than the present time. "
                                     + "The present time can be obtained using the " + "presentTime() method.");
            return;
            // I can't be scheduled, TimeInstant has already passed.
     ...
     */
    protected void scheduleWithPreempt(Entity who1, Entity who2, Entity who3,
                                       EventOf3Entities<?, ?, ?> what) throws SuspendExecution {
        ...
        EventNote note = new EventNote(who1, who2, who3, what, presentTime(), what.getSchedulingPriority(),
                                       _currentSchedulable);
    ...
    protected void scheduleWithPreempt(Entity who1, Entity who2, EventOf2Entities<?, ?> what) throws SuspendExecution {
        ...
        EventNote note = new EventNote(who1, who2, null, what, presentTime(), what.getSchedulingPriority(),
                                       _currentSchedulable);
    ...
    void preemptSimProcess(EventNote preemptNote) throws SuspendExecution {
        ...
        if (_currentProcess == null) {
            myExperiment.sendWarning("Can't preempt current SimProcess! " + "Command ignored.",
                                     "Scheduler : " + getName() + " Method: preemptSimProcess(EventNote preemptNote)",
                                     "The event-note reference passed as parameter is a null " + "reference.",
                                     "Always make sure to use valid references only.");
        ...
        EventNote currentNote = new EventNote(_currentProcess, null, null, null, presentTime(), Integer.MAX_VALUE,
                                              _currentSchedulable);
