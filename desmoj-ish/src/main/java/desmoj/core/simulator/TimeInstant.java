...
public final class TimeInstant implements Comparable<TimeInstant> {
    ...
    public TimeInstant(double time, TimeUnit unit) {
        if (unit == null) { // no time unit given
            ...
            new desmoj.core.report.ErrorMessage(null, "Can't create TimeInstant object! Simulation aborted.",
        ...
        if (unit.compareTo(TimeOperations.getEpsilon()) < 0) {
            ...
            if (TimeOperations.getStartTime() != null) {// Start time has been
                ...
                - unit.convert(TimeOperations.getStartTime()// Achtung-negative
                                             // werden
                                             // positiv
                                             .getTimeInEpsilon(), TimeOperations.getEpsilon());
     ...
     * method available through calling the class <code>TimeInstant</code> i.e. <code>TimeInstant.isEqual(a,b)</code>
     ...
     * @return: int: Returns a negative integer, zero, or a positive integer as this TimeInstant is before, at the same
     * time, or after the given TimeInstant.
     */
    @Override
     ...
     * 11:23:00:000 (begin of current day).<br\> Note that days are interpreted with respect to this instant's preferred
     * time zone.
     *
