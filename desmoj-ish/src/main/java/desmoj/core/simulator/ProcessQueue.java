...
public class ProcessQueue<P extends SimProcess> extends QueueBased implements Iterable<P> {
    ...
    public P first(Condition<P> c) {
        ...
        if (c == null) {
            sendWarning("Can not return first SimProcess fulfilling the condition!",
                        "ProcessQueue : " + getName() + " Method: void first(Condition c)",
    ...
    public boolean insert(P p) {
        ...
        if (p == null) { // null returns with warning
            sendWarning("Can not insert SimProcess!",
                        "ProcessQueue : " + getName() + " Method: boolean insert" + "(P p)",
        ...
        if (currentlySendDebugNotes()) {
            sendDebugNote("inserts " + p.getQuotedName() + " in the ProcessQueue:<br>" + _ql.toString());
        }
        ...
        if (currentlySendTraceNotes()) {
            if (p == currentEntity() && currentEntityAll().size() == 1) {
                ...
                sendTraceNote("inserts " + p.getName() + " into " + this.getQuotedName());
            }
        }
    ...
    public boolean insertAfter(P p, P after) {
        ...
        if (p == null) {
            sendWarning("Can not insert SimProcess!",
                        "ProcessQueue : " + getName() + " Method: boolean insertAfter(P p, P after)",
                        "The SimProcess 'p' given as parameter is a null reference!",
                        "Check to always have valid references when enqueueing Entities");
            return false; // no proper parameter
        }
        ...
        if (after == null) {
            sendWarning("Can not insert SimProcess!",
                        "ProcessQueue : " + getName() + " Method: boolean insertAfter(P e, P after)",
        ...
        if (currentlySendTraceNotes()) {
            if (p == currentEntity() && currentEntityAll().size() == 1) {
                ...
                sendTraceNote("inserts " + p.getName() + " into " + this.getQuotedName() + " after " + after.getName());
            }
        }
    ...
    public boolean insertBefore(P p, P before) {
        ...
        if (p == null) {
            sendWarning("Can not insert SimProcess!",
                        "ProcessQueue : " + getName() + " Method: boolean insertBefore(P p, P before)",
                        "The SimProcess 'p' given as parameter is a null reference!",
                        "Check to always have valid references when enqueueing Entities");
            return false; // no proper parameter
        }
    ...
    public P last(Condition<P> c) {
        ...
        if (c == null) {
            sendWarning("Can not insert SimProcess!",
                        "ProcessQueue : " + getName() + " Method: SimProcess last(Condition c)",
                        "The Condition -c- given as parameter is a null reference!",
                        "Check to always have valid references when querying Queues.");
            return null; // no proper parameter
        }
    ...
    public P pred(P p) {
        ...
        if (p == null) {
            sendWarning("Can not find predecessor of SimProcess in Queue!",
                        "ProcessQueue : " + getName() + " Method: SimProcess pred(P p)",
                        "The SimProcess 'p' given as parameter is a null reference!",
    ...
    public P pred(P p, Condition<P> c) {
        ...
        if (p == null) {
            sendWarning("Can not find predecessor of SimProcess in Queue!",
                        "ProcessQueue : " + getName() + " Method: SimProcess pred(P p, Condition c)",
                        "The SimProcess 'p' given as parameter is a null reference!",
                        "Check to always have valid references when querying for Entities");
            return null; // no proper parameter
        }
        ...
        if (c == null) {
            sendWarning("Can not return previous SimProcess fulfilling condition!",
                        "ProcessQueue : " + getName() + " Method: SimProcess pred(P p, Condition c)",
                        "The Condition 'c' given as parameter is a null reference!",
                        "Check to always have valid references when querying Queues.");
            return null; // no proper parameter
    ...
    public void remove(SimProcess p) {
        ...
        if (!_ql.remove((P) p)) { // watch out, removes SimProcess as a side
            ...
            sendWarning("Can not remove SimProcess from Queue!",
                        ...
                        + "this queue!",
                        "Make sure the SimProcess is inside the queue you want it " + "to be removed.");
        ...
        if (currentlySendDebugNotes()) {
            sendDebugNote("remove " + p.getQuotedName() + "<br>" + _ql.toString());
        }
        ...
        if (currentlySendTraceNotes()) {
            if (p == currentEntity() && currentEntityAll().size() == 1) {
                ...
                sendTraceNote("removes " + p.getQuotedName() + " from " + this.getQuotedName());
            }
        }
    ...
    public P removeFirst(Condition<P> c) {
        ...
        if (c == null) {
            sendWarning("Can not remove the first process fulfilling a condition!",
    ...
    public P succ(P p) {
        ...
        if (p == null) {
            sendWarning("Can not find successor of SimProcess in Queue!",
                        "ProcessQueue : " + getName() + " Method: SimProcess succ(P p)",
                        "The SimProcess 'p' given as parameter is a null reference!",
                        "Check to always have valid references when querying for " + "Entities");
            return null; // no proper parameter
        }
    ...
    public P succ(P p, Condition<P> c) {
        ...
        if (p == null) {
            sendWarning("Can not find predecessor of SimProcess in Queue!",
                        "ProcessQueue : " + getName() + " Method: SimProcess succ(P p, Condition c)",
                        "The SimProcess 'p' given as parameter is a null reference!",
                        "Check to always have valid references when querying for Entities");
            return null; // no proper parameter
        }
