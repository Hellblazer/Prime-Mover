 ...
 * priority and second in FIFO (first in first out) order. The statistic data of
...

{
    ...
    /**
     * Adds a new Entity to the QueueListStandardFifo. Entities are inserted
     * according to their priority in descending order. The highest priority Entity
     * will always be first in the queue. Entities with same priority are inserted
     * in FiFo order.
     *
    ...
    public void insert(E e) {
        ...
        if (!this.isEmpty()) {
            // continuously asks the predecessor (FIFO) entities if they have a lower
            // priority;
