...
public class ResourceDB {
    ...
    /**
     * AssignedResources is an inner class of <code>ResourceDB</code> to encapsulate
     * the pairs of: SimProcess and the number of resources it holds. These pairs
     * are stored in the vector <code>arrayOfAssignedResources</code>.
     */
     ...
     * encapsulate the pairs of: a resource pool <code>Res</code> and the number of
     * resources requested by a SimProcess. These pairs are stored in the vector
    ...
    private static class RequestedResources extends java.lang.Object {
         ...
         *
         * @param resPool      Res : The resource pool from which resources are
         *                     requested.
         * @param requestedRes int : The number of resources requested from the resource
         *                     pool <code>Res</code>.
         */
         ...
         * <code>Res</code>.
         *
         * @return int : The number of resources requested from the resource pool
         *         <code>Res</code>.
         */
        ...
        /**
         * Returns the resource pool <code>Res</code> from which a number of resources
         * are requested.
         *
         * @return Res : The resource pool <code>Res</code> from which a number of
         ...
         *
         * @param newQuantity int : The new number of resources requested from the
     ...
     * hashtable) a Vector holding all the pairs of a SimProcess and the number of
     * resources the SimProcess holds at the moment (see the inner class
     * <code>AssignedResources</code>).
     */
    private Hashtable<Res, Vector<AssignedResources>> _assignmentTable;

    /**
     * Indicates whether a cycle in the resource allocation graph is found or not.
     * Is set to <code>true</code> if a cycle (and therefore a possible deadlock) is
     * found and <code>false</code> if no cycle is found.
     */
    private boolean _cycleFound;

    /**
     * Stores all the SimProcesses which are involved in a deadlock. Is filled when
     * the method <code>checkForDeadlock()</code> is called.
     */
    private Vector<SimProcess> _deadlockedProcs;

    /**
     * Stores all the resource pools (<code>Res</code>) which are involved in a
     * deadlock. The Vector is filled when the method
    ...
    /**
     * Stores all the visited SimProcesses when traversing the resource allocation
     ...
     * capactiy of resources in the Res pool and the returned units of that Res pool
     * due to reduction (the units which might be returned in the future because
     * they are not stuck in a deadlock).
     */
    ...
    /**
     * Stores for every SimProcess the resource pool (<code>Res</code>) he is
     * requesting resources from and the number of requested resources (see the
     * inner class <code>RequestedResources</code>). The SimProcess is building the
     * key to the table entries which in turn are holding objects of the inner class
     * <code>RequestedResources</code>.
     */
    private Hashtable<SimProcess, RequestedResources> _requestTable;

    /**
     * Stores the cycle in the resource allocation graph as a string representation.
     * Is built up as the graph containing the cycle (deadlock) is drawn.
     */
    private StringBuffer _resAllocGraph;

    /**
     * Stores all the visited SimProcesses when traversing the resource allocation
     * graph. Maybe another implementation is performing better. You are invited to
     * think about it.
     */
     ...
     * to i.e. the method <code>checkProcess()</code>.
     */
    ...
    /**
     * This private constructor is defined to prevent the compiler from generating a
     * default public constructor.
     */
    /*
     * Constructor is never used private ResourceDB() {
     *
     * // make the hashtables to store all the information _assignmentTable = new
     * Hashtable<Res, Vector<AssignedResources>>(); _requestTable = new
     * Hashtable<SimProcess, RequestedResources>(); _effCapacity = new
     * Hashtable<Res, Integer>(); }
     */

    /**
     * This constructor is called from the constructor of <code>Experiment</code> to
     * make sure that for every experiment there is a new resource DB. The debug
     * output for this <code>ResourceDB</code> is turned on by default. But you have
     * to make sure, that the default output for the experiment is turned on, too.
     *
     * @param owner desmoj.Experiment : the experiment this resource database
     *              belongs to.
     */
    protected ResourceDB(Experiment owner) {
        ...
        _assignmentTable = new Hashtable<>();
        _requestTable = new Hashtable<>();
        _effCapacity = new Hashtable<>();
    ...
    /**
     * This method is called when a SimProcess can not get the resources desired to
     * check if a possible deadlock situation has occured.
     *
     * @param unsatProc desmoj.SimProcess : The SimProcess which can not get the
    ...
    public synchronized boolean checkForDeadlock(SimProcess unsatProc) {
        ...
        _where = "protected synchronized boolean checkForDeadlock(SimProcess " + "unsatProc)";

        // check for null reference
        if (!checkProcess(unsatProc, _where))
            return false; // if the SimProcess is not valid just return

        _visitedProcs = new Vector<>();
        _visitedRes = new Vector<>();
        _doneProcs = new Vector<>();
        _doneRes = new Vector<>();
        _deadlockedProcs = new Vector<>();
        _deadlockedRes = new Vector<>();
        ...
        if (_cycleFound) {
            ...
            if (_deadlockedProcs.size() == _visitedProcs.size()) {
                System.out.println("A total deadlock was detected in your model " + "at simulation time "
                ...
                + "done already) and check the debug file to learn more " + "about this deadlock.");

                sendDebugNote("A <b>total deadlock</b> situation is detected in your "
                + "simulation! <br>Examine the information provided by the " + "resource database above.");
                ...
                sendWarning("A <b>total deadlock</b> is detected in the resource "
                + "allocation graph. <br>The simulation can not continue properly due " + "to this deadlock.",
                            "ResourceDB Method: " + _where,
                            "The SimProcess '" + unsatProc.getName() + "' can not get the "
                            + "resources desired, because they are occupied by another "
                            + "SimProcess. <br>The following chain of resource allocations and "
                            + "requests has lead to the deadlock: <br>" + _resAllocGraph,
                            "Check if a situation as described above can happen in the real "
                            + "system, too. <br>Check if your model may not be implemented " + "correctly!");
            } else // no total deadlock
            {
                // check if a pending or transient deadlock is discovered
                if (additionalStatus()) {
                    // pending deadlock
                    System.out.println("A pending deadlock was detected in your model " + "at simulation time "
                    + _owner.getSimClock().getTime() + " .");
                    System.out.println("Please check the error file!");
                    System.out.println("The debug file also might help to learn more about " + "this deadlock");
                    ...
                    sendWarning("A <b>pending deadlock</b> is detected in the resource "
                    + "allocation graph. <br>Some of the processes in the "
                    + "simulation are stuck in the deadlock and can not " + "continue properly.",
                                "ResourceDB Method: " + _where,
                                "The SimProcess '" + unsatProc.getName() + "' can not get the "
                                + "resources desired, because they are occupied by another "
                                + "SimProcess. <br>The following chain of resource allocations and "
                ...
                {
                    System.out.println("A transient deadlock was detected in your model " + "at simulation time "
                    + _owner.getSimClock().getTime() + " . ");
                    System.out.println("Although this situation may self-resolve, better "
                    + "check the error and debug files!");

                    sendTraceNote("has detected a <b>transient deadlock</b> situation. <br>"
                    + "Although this situation may self-resolve, better " + "check the error and debug files!");

                    sendDebugNote("A <b>transient deadlock</b> situation is detected in your "
                    + "simulation! <br>Better examine the information provided " + "by the resource database above.");
                    ...
                    sendWarning("A <b>transient deadlock</b> is detected in the resource "
                    + "allocation graph. <br>This situation may self-resolve, "
                    + "but at the moment some processes are blocked.", "ResourceDB Method: " + _where,
                                "The SimProcess '" + unsatProc.getName() + "' can not get the "
                                + "resources desired, because they are occupied by another "
                                + "SimProcess at the moment. <br>The following chain of resource "
                                + "allocations and requests has lead to the deadlock: <br>" + _resAllocGraph
                                + "<br>Furthermore the following resource allocations and "
                                + "requests are present: <br>" + _nonCycleGraph,
                                "Check if a situation as described above can happen in the real "
                                + "system, too. <br>Check if your model may not be implemented " + "correctly!");
                }
            }
             ...
             * "to determine the resource allocation graph: " + visitedProcs.toString());
             *
             * System.out.println("The following SimProcesses are involved in the
             * " + "deadlock: " + deadlockedProcs.toString());
             */
     ...
     * Sim-process is done with it's requested number of resources from a resource
     * pool (<code>Res</code> )). The SimProcess and the number of requested
     * resources will be deleted in the corresponding Vector (of the Res pool) in
     * the assignment hashtable. The Res pool is the key to this hashtable,
     * specifying the Vector which holds all the (SimProcess/number of requested
     * resources)-pairs.
     *
     * @param resPool  desmoj.Res : The resource pool which has provided the
     *                 resources. (The key to the hashtable)
     * @param doneProc desmoj.SimProcess : The Simprocess giving back his allocated
    ...
    public void deleteResAllocation(Res resPool, SimProcess doneProc, int quantity) {
        _where = "protected void deleteResAllocation(Res resPool, " + "SimProcess doneProc, int quantity)";

        // checks for null references and negative quantity
        // if the SimProcess is not valid just return

        if (!checkProcess(doneProc, _where) || !checkRes(resPool, _where))
            return; // if the Res is not valid just return

        if (quantity <= 0) {
            sendWarning("Attempt to delete a negative or zero quantity from a "
            + "ResourceDB . The attempted action is ignored!", "ResourceDB Method: " + _where,
                        "The given quantity is zero or negative.",
                        "Make sure to only use positive quantities which are more than " + "nothing.");
        ...
        if (!_assignmentTable.containsKey(resPool)) { // if no entry -> error
            sendWarning("Attempt to delete an entry in the resource database "
            + "for a resource pool (Res), but the Res pool does not exist in the "
            + "database. The attempted action is ignored!", "ResourceDB Method: " + _where,
        ...
        } else // get hold of the corresponding Vector
        {
            ...
            for (int i = 0; i < resPoolVec.size(); i++) {
                ...
                AssignedResources assigRes = resPoolVec.elementAt(i);
                ...
                if (assigRes.getProcess() == doneProc) {
                    ...
                    if (!(assigRes.getSeizedUnits() == quantity)) {
                        ...
                        if (quantity > assigRes.getSeizedUnits()) {
                            sendWarning("Attempt to delete more resources than "
                            + "the SimProcess has allocated. Only all the formerly "
                            + "allocated resources will be deleted in the database and " + "no more!",
                                        "ResourceDB Method: " + _where,
            ...
            if (!foundInVec) {
                sendWarning("Can't find the SimProcess for which there should be "
                + "deleted allocated resources. The attempted action can't be " + "performed!",
                            "ResourceDB Method: " + _where,
                            "The entry in the database for the given SimProcess " + "can not be found.",
                            "Make sure to only delete resources which a SimProcess " + "has allocated already.");
            } // end if
    ...
    /**
     * Deletes an entry in the resource data base (should be called when a
     * Sim-process receives it's requested number of resources from a resource pool
     * (<code>Res</code> )). The resource pool and the number of requested resources
     * will be deleted in the request hashtable. The Sim-process is the key to this
     * hashtable, specifying the RequestedResources object which holds the (resource
     * pool/number of requested resources)-pair.
     *
     * @param gainProc desmoj.SimProcess : The Simprocess giving up his request for
    ...
    public void deleteResRequest(SimProcess gainProc, Res resPool, int quantity) {
        _where = "protected void deleteResRequest(SimProcess gainProc, " + "Res resPool,	int quantity)";

        // checks for null references and negative quantity
        // if the SimProcess is not valid just return

        if (!checkProcess(gainProc, _where) || !checkRes(resPool, _where))
            return; // if the Res is not valid just return
        ...
        RequestedResources reqRes = _requestTable.get(gainProc);

        // is the given resource not the same as in the database?
        if (reqRes.getResPool() != resPool) {
            sendWarning("Can't find the resource pool which requested resources "
            + "should be deleted. The attempted action can't be performed!", "ResourceDB Method: " + _where,
                        "The entry in the database for the given resource pool " + "can not be found.",
        ...
        if (quantity > reqRes.getRequestedUnits()) {
            sendWarning("Attempt to delete more requested resources than "
            + "the SimProcess has requested. Only all the formerly "
            + "requested resources will be deleted in the database and " + "no more!", "ResourceDB Method: " + _where,
                        "The entry in the database has not registered as many "
    ...
    /**
     * Makes an entry in the resource data base when a SimProcess is allocating a
     * number of resources from a resource pool (<code>Res</code>). The Sim-process
     * and the number of allocated resources will be saved in the assignment
     * hashtable. The resource pool is the key to this hashtable.
     *
     * @param resourcePool      desmoj.Res : The resource pool which is providing
     *                          the resources.
     * @param allocatingProcess desmoj.SimProcess : The Simprocess which is
     *                          allocating the resources.
     * @param quantity          int : The number of resources the SimProcess is
     *                          allocating.
     */
    public void noteResourceAllocation(Res resourcePool, SimProcess allocatingProcess, int quantity) {
        ...
        if (!checkProcess(allocatingProcess, _where) || !checkRes(resourcePool, _where))
            return; // if the Res is not valid just return
        ...
        AssignedResources assigResources = new AssignedResources(allocatingProcess, quantity);
        ...
        if (_assignmentTable.containsKey(resourcePool)) // there is an entry ...
        {
            ...
            Vector<AssignedResources> arrayOfAssigResources = _assignmentTable.get(resourcePool);
            ...
            for (int i = 0; i < arrayOfAssigResources.size(); i++) {
                // get hold of the pair (SimProcess / number of resources)
                AssignedResources alreadyAssigRes = arrayOfAssigResources.elementAt(i);
            ...
            Vector<AssignedResources> resPoolVector = new Vector<>();
    ...
    /**
     * Makes an entry in the resource data base when a SimProcess is requesting a
     * number of resources from a resource pool (<code>Res</code>). The resource
     * pool and the number of requested resources will be saved in the request
     * hashtable. The SimProcess is the key to this hashtable.
     *
     * @param requestingProcess desmoj.SimProcess : The Simprocess which is
     *                          requesting the resources.
     * @param resourcePool      desmoj.Res : The resource pool which should provide
     *                          the resources.
     * @param quantity          int : The number of resources the SimProcess wants
     *                          to get.
     */
    public void noteResourceRequest(SimProcess requestingProcess, Res resourcePool, int quantity) {
        ...
        if (!checkProcess(requestingProcess, _where) || !checkRes(resourcePool, _where))
            return; // if the Res is not valid just return
        ...
        RequestedResources reqResources = new RequestedResources(resourcePool, quantity);
        ...
        // ...
        {
            sendWarning("Attempt to make a SimProcess request resources from "
            + "more than one resource pool. The attempted action is ignored!", "ResourceDB Method: " + _where,
                        ...
                        "The SimProcess '" + requestingProcess + "' should be blocked in "
     ...
     * requesting and second for every resource pool (<code>Res</code>) all the
     * SimProcesses that are using resources from that resource pool. This
    ...
    public String toHtmlString() {
        ...
        if (_requestTable.isEmpty()) { // no SimProcess is requesting any
            ...
            for (Enumeration<SimProcess> waitingProcs = _requestTable.keys(); waitingProcs.hasMoreElements();) {
                SimProcess process = waitingProcs.nextElement();
                buffer.append("<br>SimProcess '" + process.toString() + "' is waiting on: " + "<ul>"); // start indent
                                                                                                       // list

                RequestedResources reqRes = _requestTable.get(process);

                if (reqRes == null) {
                    buffer.append("nothing!");
                } else {
                    buffer.append(reqRes.getRequestedUnits() + " resource(s) " + "from '"
                    + reqRes.getResPool().toString() + "'<br>");
                }
        ...
        // resources
        buffer.append("<u>List of all resource pools with resources used at the " + "moment:</u>");
        ...
        if (_assignmentTable.isEmpty()) {// no resources are allocated by any
            ...
            for (Enumeration<Res> usedRes = _assignmentTable.keys(); usedRes.hasMoreElements();) {
                Res resPool = usedRes.nextElement();
                buffer.append("<br>resource pool '" + resPool.toString() + "' is " + "providing: <ul>"); // start indent
                                                                                                         // list
                ...
                if (allocVector == null || allocVector.isEmpty()) {
                    ...
                    for (AssignedResources assignedRes : allocVector) {
                        buffer.append(assignedRes.getSeizedUnits() + " resource(s) " + "to '"
                        + assignedRes.getProcess().toString() + "'<br>");
                    }
                }
     ...
     * changes of state to help find bugs. The information about the simulation time
     * is extracted from the experiment and must not be given as a parameter.
     *
     * @param description java.lang.String : The description of the ResourceDB's
     *                    internal state to be passed with this debugnote
     */
    protected void sendDebugNote(String description) {

        sendMessage(new DebugNote(_owner.getModel(), _owner.getSimClock().getTime(), "resource database", description));
     ...
     * available to this ResourceDB, then the mesage is printed to the standard
     ...
     * <li><code>sendWarning()</code> to send an errormessage that does not stop the
     * experiment</li>
     * </ul>
     *
    ...
    protected void sendMessage(Message m) {
        ...
        if (m == null) {
            sendWarning("Can't send Message!", "ResourceDB Method: SendMessage(Message m)",
    ...
    protected void sendTraceNote(String description) {
        ...
        sendMessage(new TraceNote(_owner.getModel(), description, _owner.getSimClock().getTime(),
                                  _owner.getScheduler().getCurrentEntity(), _owner.getScheduler().getCurrentEvent()));
     ...
     */
    protected void sendWarning(String description, String location, String reason, String prevention) {

        // compose the ErrorMessage and send it in one command
        sendMessage(new ErrorMessage(_owner.getModel(), description, location, reason, prevention,
                                     _owner.getSimClock().getTime()));

    }

    /**
     * Checks the additional status of a deadlock, i.e. if the deadlock is a pending
     * or a transient one. Returns <code>true</code> if it is a pending deadlock and
    ...
    private synchronized boolean additionalStatus() {
        ...
        for (Enumeration<Res> resPools = _assignmentTable.keys(); resPools.hasMoreElements();) {
        ...
        do // do while some reduction was made
        {
            ...
            for (int i = 0; i < listOfProcs.size(); i++) {
                ...
                RequestedResources reqRes = _requestTable.get(crntProc);
                ...
                if (!crntRes.getPassBy()) {
                    ...
                    // pool
                    for (SimProcess proc = crntRes.getQueue().first(); proc != crntProc;
                    proc = crntRes.getQueue().succ(proc)) {
                        // how many resources is the proc requesting from
                        // crntRes?
                        int procReq = 0;
                        RequestedResources reqR = _requestTable.get(proc);
    ...
    private boolean checkProcess(SimProcess p, String where) {
        if (p == null) // if p is a null pointer instead of a process
        {
            ...
            + "ResourceDB . The attempted action is ignored!", "ResourceDB Method: " + where,
        ...
        /*
         * if (!isModelCompatible( p ) ) // if p is not modelcompatible { sendWarning (
         * "The process which should be inserted in the "+
         * "ResourceDB does not belong to this model. The attempted action " +
         * "is ignored!" , "ResourceDB: " + getName() + " Method: " + where ,
         * "The given process is not modelcompatible.", "Make sure that processes are
         * using only resources within" + " their model."); return false; }
         */
        return true;
    }

    /**
     * Checks whether the resource pool <code>Res</code> using the ResourceDB is a
     * valid <code>Res</code>.
     *
    ...
    private boolean checkRes(Res r, String where) {
        if (r == null) // if r is a null pointer instead of a Res
        {
            ...
            + "ResourceDB . The attempted action is ignored!", "ResourceDB: Method: " + where,
                        ...
                        "Make sure that only real resource pools (Res) are using the " + "resource database.");
            return false;
        }
        ...
        // compatibility
        /*
         * if (!isModelCompatible( r ) ) // if r is not modelcompatible { sendWarning (
         * "The Res which should be inserted in the "+ "ResourceDB does not belong to
         * this model. The attempted action " + "is ignored!" , "ResourceDB: " +
         * getName() + " Method: " + where , "The given Res is not
         * modelcompatible.", "Make sure that resource pools (Res) are using only a
         * resource " + "database within their model."); return false; }
         */
        return true;
    }

    /**
     * Try to find a cycle in the resource allocation graph starting with the vertex
     * specified in the parameter <code>SimProcess vertex</code>. If we can spot a
     * cycle in the resource allocation graph it is likely that we have found a
     * deadlock.
     *
     * @param vertex desmoj.SimProcess : The point to start the search of the
    ...
    private void findCycleProc(SimProcess vertex) {
        ...
        RequestedResources reqRes = _requestTable.get(vertex);
        ...
        if (reqRes != null) {
            ...
            if (!_visitedRes.contains(resVertex)) {
                ...
                _nonCycleGraph.append("SimProcess '" + vertex.getName() + "' is waiting for " + nReqRes
                + " unit(s) from the resource pool '" + resVertex.getName() + "' , but <br>");

                // 'draw' the cycle of the resource request graph only as no
                // cycle is found yet
                if (!_cycleFound) {
                    // draw the cycle
                    _resAllocGraph.append("SimProcess '" + vertex.getName() + "' is waiting for " + nReqRes
                    + " unit(s) from the resource pool '" + resVertex.getName() + "' , <br>");
                }
    ...
    /**
     * Try to find a cycle in the resource allocation graph starting with the vertex
     * specified in the parameter <code>Res vertex</code>. If we can spot a cycle in
     * the resource allocation graph it is likely that we have found a deadlock.
     *
     * @param vertex desmoj.Res : The point to start the search of the resource
    ...
    private void findCycleRes(Res vertex) {
        ...
        if (assigToProcVec != null) {
            ...
            for (AssignedResources assigRes : assigToProcVec) {
                ...
                if (!_visitedProcs.contains(procVertex)) {
                    ...
                    _nonCycleGraph.append(nAssigRes + " unit(s) from '" + vertex.getName() + "' is/are currently "
                    ...
                    if (!_cycleFound) {
                        // draw the cycle
                        _resAllocGraph.append("but " + nAssigRes + " unit(s) from '" + vertex.getName()
                        + "' is/are currently " + "used by '" + procVertex.getName() + "' and <br>");
                    }
                ...
                } else // the vertex is visited
                {
                        ...
                        !_cycleFound) // no cycle is found so far
                    {
                        ...
                        // graph
                        _resAllocGraph.append("but " + nAssigRes + " unit(s) from '" + vertex.getName()
                        + "' is/are currently " + "used by '" + procVertex.getName() + "'.<br>");
                    } // end inner if
    ...
    /**
     * Calculates the new effective available capacity of all resources the given
     * SimProcess holds units from. The given SimProcess will return the resources
     * he holds in the future, so the returning resource units will be added to the
     * effective available capacity of the resources.
     *
     * @param process desmoj.SimProcess : the SimProcess which will return the
    ...
    private void reduce(SimProcess process) {
        ...
        for (Resource resource : usedResVec) {
            // get the resource
