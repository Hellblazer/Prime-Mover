...
public class ResourceDB {
     ...
     * to the table entries which in turn are holding objects of the inner class
    ...
    /**
     * This constructor is called from the constructor of <code>Experiment</code> to make sure that for every experiment
     * there is a new resource DB. The debug output for this <code>ResourceDB</code> is turned on by default. But you
     * have to make sure, that the default output for the experiment is turned on, too.
     *
    ...
    public synchronized boolean checkForDeadlock(SimProcess unsatProc) {
        ...
        if (_cycleFound) {
            ...
            if (_deadlockedProcs.size() == _visitedProcs.size()) {
                System.out.println(
                "A total deadlock was detected in your model " + "at simulation time " + _owner.getSimClock().getTime()
                + " .");
                ...
                sendWarning("A <b>total deadlock</b> is detected in the resource "
                            ...
                            + "to this deadlock.", "ResourceDB Method: " + _where,
            ...
            } else // no total deadlock
            {
                ...
                if (additionalStatus()) {
                    // pending deadlock
                    System.out.println(
                    "A pending deadlock was detected in your model " + "at simulation time " + _owner.getSimClock()
                                                                                                     .getTime() + " .");
                    System.out.println("Please check the error file!");
                    System.out.println("The debug file also might help to learn more about " + "this deadlock");
                ...
                {
                    System.out.println(
                    "A transient deadlock was detected in your model " + "at simulation time " + _owner.getSimClock()
                                                                                                       .getTime()
                    + " . ");
                    System.out.println(
    ...
    public void deleteResAllocation(Res resPool, SimProcess doneProc, int quantity) {
        ...
        if (quantity <= 0) {
            sendWarning(
            "Attempt to delete a negative or zero quantity from a " + "ResourceDB . The attempted action is ignored!",
            "ResourceDB Method: " + _where, "The given quantity is zero or negative.",
    ...
    public void deleteResRequest(SimProcess gainProc, Res resPool, int quantity) {
        ...
        if (quantity > reqRes.getRequestedUnits()) {
            sendWarning(
            "Attempt to delete more requested resources than " + "the SimProcess has requested. Only all the formerly "
            ...
            "Make sure to only delete the same quantity of requested " + "resources as there were requested once.");
    ...
    public void noteResourceAllocation(Res resourcePool, SimProcess allocatingProcess, int quantity) {
        ...
        if (quantity <= 0) {
            sendWarning(
            "Attempt to insert a negative or zero quantity into a " + "ResourceDB . The attempted action is ignored!",
            "ResourceDB Method: " + _where, "The given quantity is zero or negative.",
            "Make sure to only use positive quantities which are more than " + "nothing.");
            return; // if the quantity is negative or zero just return
        }
        ...
        if (_requestTable.isEmpty()) { // no SimProcess is requesting any
            ...
            for (Enumeration<SimProcess> waitingProcs = _requestTable.keys(); waitingProcs.hasMoreElements(); ) {
                ...
                if (reqRes == null) {
                    ...
                    buffer.append(
                    reqRes.getRequestedUnits() + " resource(s) " + "from '" + reqRes.getResPool().toString() + "'<br>");
                }
            ...
            for (Enumeration<Res> usedRes = _assignmentTable.keys(); usedRes.hasMoreElements(); ) {
                ...
                if (allocVector == null || allocVector.isEmpty()) {
                    ...
                    for (AssignedResources assignedRes : allocVector) {
                        buffer.append(
                        assignedRes.getSeizedUnits() + " resource(s) " + "to '" + assignedRes.getProcess().toString()
                        + "'<br>");
                    }
                }
    ...
    protected void sendWarning(String description, String location, String reason, String prevention) {
        ...
        sendMessage(
        new ErrorMessage(_owner.getModel(), description, location, reason, prevention, _owner.getSimClock().getTime()));

    }

    /**
     * Checks the additional status of a deadlock, i.e. if the deadlock is a pending or a transient one. Returns
     * <code>true</code> if it is a pending deadlock and
        ...
        do // do while some reduction was made
        {
            ...
            for (int i = 0; i < listOfProcs.size(); i++) {
                ...
                if (!crntRes.getPassBy()) {
                    ...
                    // pool
                    for (SimProcess proc = crntRes.getQueue().first(); proc != crntProc; proc = crntRes.getQueue().succ(
                    proc)) {
    ...
    private boolean checkProcess(SimProcess p, String where) {
        ...
        {
            sendWarning(
            "Attempt to insert a non existing process into a " + "ResourceDB . The attempted action is ignored!",
            "ResourceDB Method: " + where, "The given process is only a null pointer.",
    ...
    private boolean checkRes(Res r, String where) {
        ...
        {
            sendWarning(
            "Attempt to insert a non existing Res into a " + "ResourceDB . The attempted action is ignored!",
            "ResourceDB: Method: " + where, "The given Res is only a null pointer.",
    ...
    private void findCycleProc(SimProcess vertex) {
        ...
        if (reqRes != null) {
            ...
            if (!_visitedRes.contains(resVertex)) {
                ...
                _nonCycleGraph.append(
                "SimProcess '" + vertex.getName() + "' is waiting for " + nReqRes + " unit(s) from the resource pool '"
        ...
        if (assigToProcVec != null) {
            ...
            for (AssignedResources assigRes : assigToProcVec) {
                ...
                if (!_visitedProcs.contains(procVertex)) {
                    ...
                    _nonCycleGraph.append(
                    nAssigRes + " unit(s) from '" + vertex.getName() + "' is/are currently " + "used by '"
                    ...
                    if (!_cycleFound) {
                        // draw the cycle
                        _resAllocGraph.append(
                        "but " + nAssigRes + " unit(s) from '" + vertex.getName() + "' is/are currently " + "used by '"
     ...
     * number of resources it holds. These pairs are stored in the vector <code>arrayOfAssignedResources</code>.
     */
