...
public final class TimeOperations {
     ...
     * explicit declaration of a time unit like in
     ...
     * zone of the result is equal to the preferred time zone of instant passed to this method.
     *
    ...
    public static TimeSpan add(TimeSpan a, TimeSpan b) {
        if (Long.MAX_VALUE - a.getTimeInEpsilon() - b.getTimeInEpsilon() < 1) {
            ...
            new desmoj.core.report.ErrorMessage(null, "Can't add TimeSpan objects! Simulation aborted.",
        ...
        if (TimeInstant.isAfter(a, b)) {
            ...
            if (b.getTimeInEpsilon() < 0) {
                ...
                if (Long.MAX_VALUE - 1 + b.getTimeInEpsilon() < a.getTimeInEpsilon()) {
                    ...
                    throw (new desmoj.core.exception.SimAbortedException(
                    new desmoj.core.report.ErrorMessage(null, "Can't subtract TimeInstant objects! Simulation aborted.",
    ...
    public static TimeSpan divide(TimeSpan dividend, double divisor) {
        if (divisor <= 0) {
            ...
            throw (new desmoj.core.exception.SimAbortedException(
            new desmoj.core.report.ErrorMessage(null, "Can't divide TimeSpan and double value! Simulation aborted.",
                                                "Class : TimeOperations  Methode : mdivide(TimeSpan dividend, double divisor)",
                                                "Cannot devide by zero.", "Never try to devide by zero.", null)));
        }
        if (divisor < 1) {
    ...
    public static double divide(TimeSpan dividend, TimeSpan divisor) {
        if (divisor.getTimeInEpsilon() == 0) {
            // cannot divide by zero
            throw (new desmoj.core.exception.SimAbortedException(
            new desmoj.core.report.ErrorMessage(null, "Can't divide TimeSpan values! Simulation aborted.",
     ...
     *                        <code>TimeFormatter</code>s who have this parameter set to <code>false</code>. The default
     *                        time
