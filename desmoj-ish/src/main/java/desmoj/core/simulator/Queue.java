...
public class Queue<E extends Entity> extends QueueBased implements Iterable<E> {
     ...
     * @return boolean : <code>True</code> if the given <code>Entity</code> is contained in the queue;
     ...
     *
     * @return E : The first entity in the queue or <code>null</code> if the queue is empty
     */
    public E first() {
    ...
    public TimeSpan getCurrentWaitTime(Entity e) {
        ...
        if (i == null) {
            sendWarning("Cannot query waiting time!",
                        ...
                        + "queue!", "Make sure the entity is inside the queue you want it to " + "be queried.");
     ...
     * <code>false</code> will be returned. The Entity will be stored in the queue until method <code>remove(Entity
     * e)</code> is called with this specific Entity. Entities inside the queue are ordered according to their priority.
     * Higher priorities are sorted in front of lower priorities. Entities with same priority are orderer according to
     * the specified strategy. The first Entity inside the queue will always be the one with the highest priority.
     *
    ...
    public boolean insert(E e) {
        ...
        if (queueLimit <= length()) {
            ...
            if (currentlySendDebugNotes()) {
                sendDebugNote(
                "refuses to insert " + e.getQuotedName() + " because the " + "capacity limit is reached. Queue:<br>"
                + _ql.toString());
            }
    ...
    public boolean insertBefore(E e, E before) {
        ...
        if (currentlySendDebugNotes()) {
            sendDebugNote(
            "inserts " + e.getQuotedName() + " before " + before.getQuotedName() + "<br>" + _ql.toString());
        }
        ...
        if (currentlySendTraceNotes()) {
            if (e == currentEntity() && currentEntityAll().size() == 1) {
                ...
                sendTraceNote(
                "inserts " + e.getName() + " into " + this.getQuotedName() + " before " + before.getName());
            }
        }
     ...
     * Entity fulfills the given condition or the queue is empty,
     * <code>null</code> will be returned.
     *
     * @param c Condition : The condition that the entity returned must fulfill
     ...
     * specified queueing discipline. Please choose a constant from <code>QueueBased</code>
     * (<code>QueueBased.FIFO</code>,
