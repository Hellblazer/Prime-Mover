 ...
 * entities in. The sort order of the queue is determined first by the
 * priorities of the enqueued entities and second by the given sort order. The
 * default sort order is FIFO (first in, first out) but others like LIFO (last
 * in, first out) can be chosen, too. See the constants in class
...
public class Queue<E extends Entity> extends QueueBased implements Iterable<E> {
    ...
    /**
     * Counter for the entities which are refused to be enqueued, because the queue
     * capacity is full.
     */
    private long _refused;

    /**
     * Constructs a simple priority and FIFO based waiting-queue for entities with a
     * maximum capacity of 2,147,483,647 waiting entities, which should serve as an
     * approximation of infinite queues sufficiently well for most purposes.Note
     * that since SimProcesses are derived from Entities, they can be queued inside
     * this queue, too.
     * <p>
     * The usage of the generic version <code>Queue&lt;Type&gt;</code> where
     * <code>Type</code> is derived from <code>Entity</code> is recommended for type
     * safety. Using the raw type <code>Queue</code> yields a queue in which any
     * <code>Entity</code> can be enqueued, typically requiring type casting on
     * accessing entities enqueued.
     *
     ...
     */
    public Queue(Model owner, String name, boolean showInReport, boolean showInTrace) {
    ...
    /**
     * Constructs a simple priority based waiting-queue for entities, the kind of
     * queue sort order and the capacity of the queue can be chosen. Note that since
     * SimProcesses are derived from Entities, they can be queued inside this queue,
     * too.
     * <p>
     * Queues can be sorted as follows:
     * <ul>
     * <li><code>QueueBased.FIFO</code> sorts entities by their priority, highest
     * priority first. Entities with the same priority are enqueued based on
     ...
     *                     entities can be enqueued. Zero (0) can be used as
     ...
     * @param showInTrace  boolean : Flag for queue to produce trace messages
     */
    public Queue(Model owner, String name, int sortOrder, int qCapacity, boolean showInReport, boolean showInTrace) {
        ...
        default:
            ...
            + "A queue with Fifo sort order will be created.",
                        "Queue : " + getName() + " Constructor: Queue(Model owner, String name, "
    ...
    /**
     * Returns <code>true</code> if the given <code>Entity</code> is contained in
     * the queue; <code>false</code> otherwise.
     *
     * @return boolean : <code>True</code> if the given <code>Entity</code> is
    ...
    /**
     * Returns the first entity queued in this queue or <code>null</code> in case
     * the queue is empty.
     *
     * @return E : The first entity in the queue or <code>null</code> if the queue
     *         is empty
     */
    public E first() {
     ...
     * condition. The queue is searched from front to end and the first entity that
     * returns <code>true</code> when the condition is applied to it is returned by
     * this method. If no Entity fulfills the given condition or the queue is empty,
     * <code>null</code> will be returned.
     *
     * @return E : The first entity queued in this queue applying to the given
     *         condition or <code>null</code>
     * @param c Condition : The condition that the entity returned must fulfill
     */
    public E first(Condition<E> c) {

        if (c == null) {
            sendWarning("Can not return first entity fulfilling the condition!",
     ...
     *
     * @return int :The position of the entity as an <code>int</code>. Returns -1 if
     ...
     *
     * @return E : The <code>Entity</code> at the position of <code>int</code> or
    ...
    /**
     * Returns the waiting time of an entity present in the queue. If the entity
    ...
    public TimeSpan getCurrentWaitTime(Entity e) {
        ...
        if (e == null) {
            sendWarning("Cannot query waiting time!",
                        "Queue : " + getName() + " Method:  void getCurrentWaitTime(Entity e)",
                        "The Entity 'e' given as parameter is a null reference!",
                        "Check to always have valid references when querying" + "waiting durations");
            return null; // no proper parameter
        }
        ...
        if (i == null) {
            sendWarning("Cannot query waiting time!",
                        "Queue : " + getName() + " Method:  void getCurrentWaitTime(Entity e)", "The Entity 'e' ("
                        + e.getQuotedName() + ") given as parameter is not enqueued in this " + "queue!",
                        "Make sure the entity is inside the queue you want it to " + "be queried.");
            return null; // not enqueued here
        }

        if (currentlySendDebugNotes()) {
            sendDebugNote("remove " + e.getQuotedName() + "<br>" + _ql.toString());
        }
    ...
    /**
     * Enters a new Entity into the queue. If the capacity of the queue is full, the
     * entity will not be enqueued and <code>false</code> will be returned. The
     * Entity will be stored in the queue until method <code>remove(Entity e)</code>
     * is called with this specific Entity. Entities inside the queue are ordered
     * according to their priority. Higher priorities are sorted in front of lower
     * priorities. Entities with same priority are orderer according to the
     * specified strategy. The first Entity inside the queue will always be the one
    ...
    public boolean insert(E e) {
        ...
        if (e == null) { // null returns with warning
            sendWarning("Can not insert entity into Queue! Command ignored.",
                        "Queue : " + getName() + " Method: boolean insert(E e)",
                        "The Entity 'e' given as parameter is a null reference!",
        ...
        if (!isModelCompatible(e)) {
            sendWarning("Can not insert entity into Queue! Command ignored.",
                        ...
                        "Check if your submodels are allowed to mingle with other " + "model's components.");
        ...
        if (queueLimit <= length()) {
            ...
            if (currentlySendDebugNotes()) {
                sendDebugNote("refuses to insert " + e.getQuotedName() + " because the "
                + "capacity limit is reached. Queue:<br>" + _ql.toString());
            }
        ...
        if (currentlySendDebugNotes()) {
            sendDebugNote("inserts " + e.getQuotedName() + " in the queue:<br>" + _ql.toString());
        }
        ...
        if (currentlySendTraceNotes()) {
            if (e == currentEntity() && currentEntityAll().size() == 1) {
                ...
                sendTraceNote("inserts " + e.getName() + " into " + this.getQuotedName());
            }
        }
    ...
    /**
     * Enters a new Entity into the queue and places it after the given Entity. If
     * the capacity of the queue is full, the entity will not be enqueued and
     ...
     * enqueued and <code>false</code> will be returned. The Entity will be stored
     * in the queue until method <code>remove(Entity e)</code> is called with this
     * specific Entity.
     *
     * @return boolean : Is <code>true</code> if insertion was successful,
     *         <code>false</code> otherwise (i.e. capacity limit is reached).
    ...
    public boolean insertAfter(E e, E after) {
        ...
        if (e == null) {
            sendWarning("Can not insert entity into Queue! Command ignored.",
                        "Queue : " + getName() + " Method: boolean insertAfter(E e, E after)",
                        "The Entity 'e' given as parameter is a null reference!",
                        "Check to always have valid references when enqueueing Entities");
            return false; // no proper parameter
        }
        ...
        if (currentlySendDebugNotes()) {
            sendDebugNote("inserts " + e.getQuotedName() + " after " + after.getQuotedName() + "<br>" + _ql.toString());
        }

        // produce trace output
        if (currentlySendTraceNotes()) {
            if (e == currentEntity() && currentEntityAll().size() == 1) {
                sendTraceNote("inserts itself into " + this.getQuotedName() + " after " + after.getName());
            } else {
                sendTraceNote("inserts " + e.getName() + " into " + this.getQuotedName() + " after " + after.getName());
            }
        }
     ...
     * Entity. If the capacity of the queue is full, the entity will not be enqueued
     * and <code>false</code> will be returned. Make sure that the Entity given as
     * reference is already queued inside the queue, else the Entity will not be
     * queued and <code>false</code> will be returned. The Entity will be stored in
     * the queue until method <code>remove(Entity e)</code> is called with this
     * specific Entity.
     *
     * @return boolean : Is <code>true</code> if insertion was successful,
     *         <code>false</code> otherwise (i.e. capacity limit is reached).
    ...
    public boolean insertBefore(E e, E before) {
        ...
        if (e == null) {
            sendWarning("Can not insert entity into Queue! Command ignored.",
                        "Queue : " + getName() + " Method: boolean insertBefore(E e, " + "Entity before)",
                        "The Entity 'e' given as parameter is a null reference!",
                        "Check to always have valid references when enqueueing Entities");
            return false; // no proper parameter
        }
        ...
        if (currentlySendDebugNotes()) {
            sendDebugNote("inserts " + e.getQuotedName() + " before " + before.getQuotedName() + "<br>"
            + _ql.toString());
        }
        ...
        if (currentlySendTraceNotes()) {
            if (e == currentEntity() && currentEntityAll().size() == 1) {
                ...
                sendTraceNote("inserts " + e.getName() + " into " + this.getQuotedName() + " before "
                + before.getName());
            }
        }
    ...
    /**
     * Returns a boolean value indicating if the queue is empty or if any number of
     * entities is currently enqueued in it.
     *
    ...
    /**
     * Returns the last Entity queued in this queue or <code>null</code> in case the
     * queue is empty.
     *
     * @return E : The last Entity in the queue or <code>null</code> if the queue is
     *         empty
     */
    public E last() {
     ...
     * condition. The queue is searched from end to front and the first entity that
     * returns <code>true</code> when the condition is applied to it is returned. If
     * no Entity fulfills the given condition or the queue is empty,
     * <code>null</code> will be returned.
     *
     * @return E : The last Entity queued in this queue applying to the given
     *         condition or <code>null</code>
     * @param c Condition : The condition that the entity returned must fulfill
     */
    public E last(Condition<E> c) {
        ...
        if (c == null) {
            sendWarning("Can not insert entity!", "Queue : " + getName() + " Method: Entity last(Condition c)",
                        "The Condition -c- given as parameter is a null reference!",
                        "Check to always have valid references when querying Queues.");
            return null; // no proper parameter
        }
    ...
    /**
     * Returns the entity enqueued directly before the given Entity in the queue. If
     * the given Entity is not contained in this queue or is at the first position
     * thus having no possible predecessor, <code>null</code> is returned.
     *
     * @return E : The Entity directly before the given Entity in the queue or
     *         <code>null</code>.
    ...
    /**
     * Returns the entity enqueued before the given Entity in the queue that also
     * fulfills the condition given. If the given Entity is not contained in this
     * queue or is at the first position thus having no possible predecessor,
     * <code>null</code> is returned. If no other Entity before the given one
     * fulfills the condition, <code>null</code> is returned, too.
     *
     * @return E : The Entity before the given Entity in the queue fulfilling the
     *         condition or <code>null</code>.
     * @param e E : An Entity in the queue
    ...
    public E pred(E e, Condition<E> c) {
        ...
        if (e == null) {
            sendWarning("Can not find predecessor of Entity in Queue!",
                        "Queue : " + getName() + " Method: Entity pred(E e, Condition c)",
                        "The Entity 'e' given as parameter is a null reference!",
                        "Check to always have valid references when querying " + "for Entities");
            return null; // no proper parameter
        }
        ...
        if (c == null) {
            sendWarning("Can not return previous Entity fulfilling the condition!",
                        "Queue : " + getName() + " Method: Entity pred(E e, Condition c)",
                        "The Condition 'c' given as parameter is a null reference!",
                        "Check to always have valid references when querying Queues.");
            return null; // no proper parameter
    ...
    /**
     * Removes the given Entity from the Queue. If the given Entity is not in the
     * Queue, a warning will be issued but nothing else will be changed.
     *
    ...
    public void remove(Entity e) {
        ...
        if (e == null) {
            sendWarning("Can not remove Entity from Queue!", "Queue : " + getName() + " Method:  void remove(Entity e)",
                        "The Entity 'e' given as parameter is a null reference!",
                        "Check to always have valid references when removing " + "Entities");
        ...
        if (!_ql.remove((E) e)) { // watch out, already removed as a side
            ...
            sendWarning("Can not remove Entity from Queue!", "Queue : " + getName() + " Method:  void remove(Entity e)",
                        ...
                        + "queue!", "Make sure the entity is inside the queue you want it to " + "be removed.");
        ...
        if (currentlySendTraceNotes()) {
            if (e == currentEntity() && currentEntityAll().size() == 1) {
                ...
                sendTraceNote("removes " + e.getQuotedName() + " from " + this.getQuotedName());
            }
        }
    ...
    /**
     * Removes the entity queued at the given position. The first position is 0, the
     * last one length()-1.
     *
    ...
    /**
     * Removes the first entity from the queue and provides a reference to this
     * entity. If the queue is empty, <code>null</code> is returned.
     *
     * @return E : The first entity in this queue, which has been removed, or
        ...
        if (first != null) {
            ...
            if (currentlySendTraceNotes()) {
                if (first == currentEntity() && currentEntityAll().size() == 1) {
                    ...
                    sendTraceNote("removes " + first.getQuotedName() + " from " + this.getQuotedName());
                }
            }
    ...
    /**
     * Removes the first entity from the queue that fulfills the given condition.
     * Also provides a reference to this entity. If the queue does not contain an
     * entity that fulfills the condition (e.g. if the queue is empty),
    ...
    public E removeFirst(Condition<E> c) {
        ...
        if (c == null) {
            sendWarning("Can not remove the first entity fulfilling a condition!",
    ...
    /**
     * Removes the last entity from the queue and provides a reference to this
     * entity. If the queue is empty, <code>null</code> is returned.
     *
     * @return E : The last entity in this queue, which has been removed, or
     *         <code>null</code> in case the queue was empty
     */
    public E removeLast() {
        ...
        if (last != null) {
            ...
            if (currentlySendTraceNotes()) {
                if (last == currentEntity() && currentEntityAll().size() == 1) {
                    ...
                    sendTraceNote("removes " + last.getQuotedName() + " from " + this.getQuotedName());
                }
            }
    ...
    /**
     * Removes the last entity from the queue that fulfills to the given condition,
     * determined by traversing the queue from last to first until an entity
     * fulfilling the condition is found. Also provides a reference to this entity.
     * If the queue does not contain an entity that fulfills the condition (e.g. if
     * the queue is empty), <code>null</code> is returned.
     *
     * @param c Condition : The condition that the entity returned must fulfill
     *
     * @return E : The last entity in this queue fulfilling the condition, which has
     *         been removed from the queue. <code>Null</code> in case no entity
     *         fulfills the condition.
     */
    public E removeLast(Condition<E> c) {

        if (c == null) {
            sendWarning("Can not remove the last entity fulfilling a condition!",
    ...
    /**
     * Sets the sort order of this Queue to a new value and makes this Queue use
     * another <code>QueueList</code> with the specified queueing discipline. Please
     * choose a constant from <code>QueueBased</code> (<code>QueueBased.FIFO</code>,
     * <code>QueueBased.FIFO</code> or <code>QueueBased.Random</code>) The sort
     * order of a Queue can only be changed if the queue is empty.
     *
    ...
    public void setQueueStrategy(int sortOrder) {
        ...
        if (!isEmpty()) {
            sendWarning("The Queue for which the queueing discipline should be "
            + "changed is not empty. The queueing discipline will remain unchanged!",
                        ...
                        "The Queue already contains some entities ordered according a " + "certain order.",
     ...
     * Useful for queues with random sort order only; to other queues, calling this
     * method has no effect, resulting in a warning.
     *
     * @param newSeed long : new seed of the underlying queue list's pseudo random
     *                number generator
     */
    public void setSeed(long newSeed) {
        if (_ql instanceof QueueListRandom<?>) {
            ...
            sendWarning("Cannot set seed of queue!", "Queue : " + getName() + " Method: setSeed(long newSeed)",
                        "The queue does not randomize entries.", "Make sure to call setSeed(long newSeed) "
    ...
    /**
     * Returns the entity enqueued directly after the given Entity in the queue. If
     * the given Entity is not contained in this queue or is at the last position
     * thus having no possible successor, <code>null</code> is returned.
     *
     * @return E : The Entity directly after the given Entity in the queue or
     *         <code>null</code>.
     * @param e E : An Entity in the queue
     */
    public E succ(E e) {
     ...
     * fulfills the condition given. If the given Entity is not contained in this
     * queue or is at the last position thus having no possible successor,
     * <code>null</code> is returned. If no other Entity after the given one
     * fulfills the condition, <code>null</code> is returned, too.
     *
     * @return E : The Entity after the given Entity in the queue fulfilling the
     *         condition or <code>null</code>.
     * @param e E : An Entity in the queue
    ...
    public E succ(E e, Condition<E> c) {
        ...
        if (e == null) {
            sendWarning("Can not find predecessor of Entity in Queue!",
                        "Queue : " + getName() + " Method: Entity succ(E e, Condition c)",
                        "The Entity 'e' given as parameter is a null reference!",
                        "Check to always have valid references when querying for Entities");
            return null; // no proper parameter
        }
