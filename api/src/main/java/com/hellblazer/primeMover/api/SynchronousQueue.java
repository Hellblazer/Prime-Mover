 ...
 * You cannot <tt>peek</tt> at a synchronous queue because an element is only present when you try to remove it; you
...
public interface SynchronousQueue<E> extends Queue<E> {
     ...
     * collection <tt>c</tt> may result in elements being in neither, either or both collections when the associated
     ...
     * failure encountered while attempting to add elements to collection <tt>c</tt> may result in elements being in
     ...
     * Returns <tt>true</tt> if this queue contained the specified element (or equivalently, if this queue changed as a
     * result of the call).
     *
